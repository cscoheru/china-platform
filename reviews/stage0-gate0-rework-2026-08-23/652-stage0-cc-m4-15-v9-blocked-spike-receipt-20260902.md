...
### §RED_LINE_AUDIT.1 653-A.0 P4-A.0 规范 v2 落地 (per 652 审计 P4-1 处置; 2026-09-02 立; 653 沿用)

**审计 P4 发现** (per Cursor 审验端 PART 1 §D; rev90 status 行第三型自指陈旧复发):
- 652 §CURRENT status 行 pin 中间 SHA `04721b7` 为"终态 HEAD"——实际终态 `5537aff`（写入时已至少落后 2 commits）。
- "待 §C-5 双推复核"字样在 0a3d284 复核通过后未清除——第三型自指陈旧。
- 与执行端本刀自己固化的 P4-1 规则（docs/75:283"中间 SHA 一律不入 status 文本"）直接冲突（讽刺点已入审计记录）。
- §NOW 已正确收口，实质无损 → rev91 修正 + 653-A.0 规范 v2。
...
- **沿用 652-A.0 P4-2 amend-first 规则**（先 amend 完成再写链文本；cc_head 链 SHA 一律 `git log --format=%H -n <n>` 实测，禁记忆/预写）。

**653-C 执行节奏**: rev92 §NOW 刷新 commit 内**同时**完成 status 行收口 + §NOW 收口 + "待复核"字样清除（一次原子提交，per 规范 v2）。

— End §RED_LINE_AUDIT.1 tailnote —
