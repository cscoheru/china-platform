# 623-stage0-architect-s622-o1-§5.2.x-real-sha-locked-江苏样本-地市第十刀-tasking-20260830

> **任务书类型**: 架构师签发 (per ARCH-PULSE step 3 verbatim 583/585/587/589/591/593/594/595/596/597/598/599/600/601/602/603/604/605/606/607/608/609/610/611/612/613/614/615/616/617/618/619/620/621/622 precedent)
> **触发依据**: 623 audit PASS → 架构师 step 3 签发下一刀 per ARCH-PULSE step 3 verbatim (status=AUDITED + 无新 PENDING)
> **前置**: 623 audit PASS（14 维度全 PASS + 1 ⚠ ACCEPTED + 零 FAIL；三侧100%收敛 feat(622) `ae25ee9` + cc_head(622) backfill `88fe2a5` + §双推 populate `2abe7ed` + status `53b01930e11dad67da422c3605955e01a4ba677f` → HEAD=origin=github=`53b01930e11dad67da422c3605955e01a4ba677f`；cc_head queue pointer `2abe7ed`；江苏样本链路 9/15 节点；manifest INVARIANT 996 == 996 == 996 ✓；14 受保护文件零漂移；31+/31+ 红线 100% 兑现；零真实 paddleocr API 调用；零 `--confirm-*` 字面；零 `--enable-cloud-ocr=PROVIDER` 字面；零用户裁定 / 零用户亲验 / 零用户动作；B 路（公开源自动获取 per docs/52）保持主路径；A 路（用户投递 per docs/51）保留为 fallback 标注）+ 622 receipt DELIVERED + 621 audit (= 622 audit PASS) + 620 audit PASS + 619 receipt DELIVERED + 618 audit PASS + 617 receipt PASS + 617 audit PASS + 616 audit PASS + 616 receipt PASS + 615 audit FAIL 614 修复闭环 + 614 receipt DELIVERED + 613 audit PASS + 612 receipt PASS + 611 audit PASS + 610 receipt PASS + 609 audit PASS + 608 receipt PASS + 607 audit PASS + 606 receipt PASS + 605 audit PASS + 604 audit PASS + 603 + 602 + 601 + 600/599/597/595/594/593/591/589/587/585/583 全链 PASS
> **签发时间**: 2026-08-30
...
**O1 §5.2.x 江苏样本第十刀（地市样本第十刀）落地**

接续 605 + 606 + 608 + 610 + 612 + 617 + 619 + 621 + 622 江苏样本链路 9/15 → 10/15（地市样本 9/10）；不解决 O1 整体收口（O1 整体仍 WAITING_FILE per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；待全部 5 省样本 + 10 地市样本均 SHA-locked 后另刀审议）。

### 0.2 候选清单（per 622 receipt §9 候选清单 #2 verbatim + 623 audit §7 候选 #2 verbatim + 622 audit §7 候选 #2 verbatim + 621 audit §7 优先级 2 verbatim + 620 audit §7 候选 #2 verbatim + 619 receipt §9 候选 #1 verbatim + 618 audit §7.2 优先级 2 verbatim）

| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.suqian.gov.cn/` | 200 OK | ≥ 1 KB | **首选** = 宿迁市统计局首页 per 622 receipt §9 候选清单 #2 verbatim |
| `https://tjj.zhenjiang.gov.cn/` | 200 OK | ≥ 1 KB | fallback #1 = 镇江市统计局首页（已用 per 621，本刀不重复首选）|
| `https://tjj.taizhou.gov.cn/` | 200 OK | ≥ 1 KB | fallback #2 = 泰州市统计局首页（已用 per 622，本刀不重复首选）|

**首选采用 = tjj.suqian.gov.cn 宿迁市统计局首页**
...
- 用户授权 #1 仍生效无需二次授权（per 622 §0.2 + 623 audit PASS precedent + 621 §0.2 + 619 §0.2 + 617 §0.1 precedent）
- 零 `--confirm-*` 字面 / 零 `--enable-cloud-ocr=PROVIDER` 字面
- 零 OCR threshold lowering（gate_thresholds.json 3709 bytes 不变）
- 零公网爬网（仅 tjj.suqian.gov.cn 政府/统计局域）
- 零 重新宣告 O3 整体 CLOSED（per 二十六重声明 + 623 不二次宣告）
...
- 零 修改 source_registry/registry.csv 既有 11 行（既有 11 行 SHA `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变；623 仅 +1 行 bytes 总数变化是预期 per ⚠ disclosure）
...
- 零 修改 622 audit PASS (= 623 audit PASS) 文件（架构师自签；执行端零修改；仅随 623 commit 入库 per docs 房规）
- 零 修改 619 receipt / 620 receipt / 621 receipt / 622 receipt 实质内容
...
- 零 真实 PDF 上传（非 seed_archives/；仅 `data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html` 落）
...
- 623 audit PASS（14 维度全 PASS + 1 ⚠ ACCEPTED + 零 FAIL；三侧100%收敛 feat(622) `ae25ee9` + cc_head(622) backfill `88fe2a5` + §双推 populate `2abe7ed` + status `53b01930e11dad67da422c3605955e01a4ba677f` → HEAD=origin=github=`53b01930e11dad67da422c3605955e01a4ba677f`；cc_head queue pointer `2abe7ed`）✓
- 622 receipt DELIVERED（8-segment delivery all landed + 1 ⚠ ACCEPTED + 零 FAIL）✓
...
执行端自取 tjj.suqian.gov.cn 宿迁市统计局公开源 per 622 receipt §9 候选清单 #2 verbatim + 623 audit §7 候选 #2 verbatim + 622 audit §7 候选 #2 verbatim + 621 audit §7 优先级 2 verbatim + 620 audit §7 候选 #2 verbatim + 619 receipt §9 候选 #1 verbatim + 618 audit §7.2 优先级 2 verbatim + 2026-08-29 治理铁律「数据源唯一=政府/统计局/研究机构自取」+ 用户授权 #1 仍生效无需二次授权。

首选探测（per 622 receipt §9 候选清单 #2 verbatim）：
...
| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.suqian.gov.cn/` | 200 | ≥ 1 KB | **首选采用** = 宿迁市统计局首页 |
| `https://tjj.zhenjiang.gov.cn/` | 200 | ≥ 1 KB | fallback #1 = 镇江市统计局首页（已用 per 621）|
| `https://tjj.taizhou.gov.cn/` | 200 | ≥ 1 KB | fallback #2 = 泰州市统计局首页（已用 per 622）|

**采用 = tjj.suqian.gov.cn 宿迁市统计局首页**（per 0.2 候选清单）

零 `--confirm-*` 字面 ✓
零用户动作 ✓
零用户裁定 ✓
执行端零爬网公网（非政府域）✓（仅 tjj.suqian.gov.cn 政府/统计局域）
...
- `source_registry/registry.csv` +1 行（line count 16 → 17）：
  - 新行 = `tjj.suqian.gov.cn,宿迁市统计局,MUNICIPAL_BULLETIN,...,data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html,<sha>,<bytes>,S0,...`
...
⚠ disclosure（已知 + ACCEPTED per 623 §0.2 verbatim）：source_registry/registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数 per 606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622 precedent）
...
- source_document 行新增 `doc_kind='OCR_SCAN'` + `source_sha256='<sha>'` + `archive_path='data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html'`
...
### (E) docs/45 §6.2 O1 status append（per 623 · 2026-08-30）

落地：
- docs/45 line 569+ append `> ⚠ **docs/45 §6.2 O1 status append**（per 623 · 2026-08-30）：O1 §5.2.x 江苏样本第十刀（地市样本第十刀）已落地（`<sha-prefix>` per source_registry/registry.csv +1 行；tjj.suqian.gov.cn 宿迁市统计局首页 `<bytes>` per 623 §0.2 候选清单 #1 首选采用；执行端自取预 vetted 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；paddle-ocr e2e 在 .venv-paddle 隔离 venv 内接通 + HTML 路径走 docs/53 §5 connector 模式 + source_document + lineage JSONB mock writer 9 字段完整 + migration 001-013 零触碰 + 既有 11 行 SHA 零漂移）；江苏样本链路 10/15 节点；后续江苏样本刀待续接。docs 房规 NOT-IN-MANIFEST。`
...
### (G) manifest bump K=4 → 996 → 1000

触发：(A)(B)(C)(D)(E)(F) 全部 PASS

落地：
- `scripts/_knife623_manifest_bump.py` NEW spike_helper +1
- 623 audit PASS `623-stage0-architect-s622-o1-§5.2.x-real-sha-locked-江苏样本-地市第九刀-tasking-20260830-audit-PASS-20260830.md` (审计 OF 622 tasking 江苏样本-地市第九刀) 入库随 623 commit (per docs 房规「审计文件不单独 commit 随下一刀入库」= 623 audit 是 623 commit 的随附) NEW documentation +1
- 623 receipt（本文件落地后）NEW documentation +1
- 江苏样本地市第十刀 HTML `data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth +1
- source_registry/registry.csv REFRESH（file-based role_count 守门不增计数 per 606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622 precedent；+1 行 bytes 总数变化是预期 per ⚠ disclosure；既有 11 行 SHA 不变）
...
- 14 受保护文件（13 既有 + 江苏样本地市第十刀 HTML + 623 audit PASS 入库随 623 commit + 623 receipt 自身）
...
- `extracts/` dir 不变 ✓
- `registry.csv` 既有 11 行 sha `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变（623 +1 行 bytes 总数变化是预期 per ⚠ disclosure）✓
...
- `data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth

**31+ 红线 100% 兑现** (per 623 §0.3 + 2026-08-29 治理铁律)：详同 §0.3 红线清单。

**⚠ disclosures (1 项 ACCEPTED per 623 §0.2)**：
**⚠ #1 (source_registry/registry.csv +1 行)**: per 623 §0.2 ⚠ disclosure — registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数）✓

**登记→实装闭环 = 583 → 584 BLOCKED → 585 → 587 → 589 → 591 → 593 → 594 → 595 → 596 → 597 → 598 → 599 → 600 → 601 → 602 → 603 → 604 → 605 → 606 → 607 → 608 → 609 → 610 → 611 → 612 → 613 → 614 → 615 → 616 → 617 → 618 → 619 → 620 → 621 → 622 → 623**（623 既闭合 O1 §5.2.x 江苏样本第十刀（地市样本第十刀；宿迁市统计局）落地（执行端自取 tjj.suqian.gov.cn 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；接续 605 + 606 + 608 + 610 + 612 + 617 + 619 + 621 + 622 江苏样本链路 9/15 → 10/15；江苏样本链路 10/15 节点）+ docs/45 §6.2 O1 status append + docs/49/50/51/52/53 F 段 SKIP + 江苏样本地市第十刀 SHA-locked HTML + source_registry/registry.csv +1 行（既有 11 行 SHA 不变）+ 14 受保护文件零漂移 + 31+ 红线 100% 兑现 + 1 ⚠ disclosure ACCEPTED）

---

## §2. 任务书交付预期完成条件（架构师验收清单）

- 623 receipt 含 (A)(B)(C)(D)(E)(F)(G)(H) 八段交付完整
- 623 receipt 含 4 步 commit 链（feat + cc_head backfill + populate + status）落地 + 三侧 100% 收敛
- 623 receipt 含 Manifest INVARIANT 1000 == 1000 == 1000 ✓
- 623 receipt 含 14 受保护文件零漂移清单
- 623 receipt 含 31+ 红线 100% 兑现清单
- 623 receipt 含 ⚠ disclosures (1 项 ACCEPTED)
- 623 receipt §9 含下一刀候选清单（per 623 audit 待签发）
...
5. 是否真实 PDF 上传（非 seed_archives/）？→ 不上传（仅 `data/seed_archives/jiangsu_suqian_tjj_gov_cn_20260830.html` 落）
...
- 623 既续接 622 江苏样本地市第九刀（泰州）→ 江苏样本地市第十刀（宿迁）
- 605 + 606 + 608 + 610 + 612 + 617 + 619 + 621 + 622 + 623 = 江苏样本链路 10/15 节点
...
**624 tasking 候选**（per 623 audit §7 + 622 receipt §9 + 622 audit §7 + 621 receipt §9 优先级 2 verbatim）：
- 候选 #1：623 receipt 审计刀（per 583/585/587/.../622 audit precedent）
- 候选 #2：O1 §5.2.x 江苏样本省样本第二刀（其它省统计局公开源：如浙江 / 广东 / 山东等省统计局公开源；接续 605 首批省样本链路 1/5 → 2/5）
- 候选 #3：O1 §5.2.x 江苏样本省样本第三/四/五刀（其它省统计局公开源，续接 605 + 624 链路 3/5 → 5/5）
...
- 架构师 (Architect) — 623 tasking 签发
- 签发时间：2026-08-30
- 本任务书 NOT-IN-MANIFEST per docs 房规（任务书不单独 commit 随下一刀入库）
- queue §CURRENT status: AUDITED → **PENDING** + note「623 tasking 签发 · O1 §5.2.x 江苏样本第十刀（地市样本第十刀；首选 tjj.suqian.gov.cn 宿迁市统计局）」

---

— End of `623-stage0-architect-s622-o1-§5.2.x-real-sha-locked-江苏样本-地市第十刀-tasking-20260830.md` —
