# 622-stage0-cc-o1-§5.2.x-real-sha-locked-江苏样本-地市第九刀-tasking-20260830-receipt

> **回执类型**: 执行端交付 → 架构师审计 (per ARCH-PULSE step 4 verbatim 583/585/587/589/591/593/594/595/596/597/598/599/600/601/602/603/604/605/606/607/608/609/610/611/612/613/614/615/616/617/618/619/620/621 平行模式；本终端合并架构师+执行端角色 per 用户 2026-08-30 指令「取消执行端唤醒，全部为本终端执行」)
> **触发依据**: 622 tasking §0.1 verbatim 落地 → 执行端 ACK + (A)(B)(C)(D)(E)(F)(G)(H) 八段交付
> **前置**: 621 audit (= 622 audit PASS) 落地（14 维度全 PASS + 1 ⚠ ACCEPTED + 零 FAIL；三侧 100% 收敛 feat(621) `6fab670` + cc_head(621) backfill `27cf955` + §双推 populate `7b8d4ce` + status `64a8c2bff7638661f410d03c89c83be18fc5c095` → HEAD=origin=github=`64a8c2bff7638661f410d03c89c83be18fc5c095`；cc_head queue pointer `7b8d4ce`）+ 622 tasking 签发（untracked；本终端合并执行角色后落地）+ 621 audit PASS + 620 audit PASS + 619 receipt DELIVERED + 618 audit PASS + 617 receipt PASS + 617 audit PASS + 616 audit PASS + 616 receipt PASS + 615 audit FAIL 614 修复闭环 + 614 receipt DELIVERED + 613 audit PASS + 612 receipt PASS + 611 audit PASS + 610 receipt PASS + 609 audit PASS + 608 receipt PASS + 607 audit PASS + 606 receipt PASS + 605 audit PASS + 604 audit PASS + 603 + 602 + 601 + 600/599/597/595/594/593/591/589/587/585/583 全链 PASS
> **交付时间**: 2026-08-30
...
**首选探测** (per 622 tasking §1.1 verbatim):

```
$ curl -L --max-time 30 -s -o /tmp/622_discover.html \
    -w "HTTP %{http_code} | bytes=%{size_download}" https://tjj.taizhou.gov.cn/
HTTP 200 | bytes=34117
```

**实测候选探测** (按 622 tasking §0.2 候选清单顺序):

| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.taizhou.gov.cn/` | 200 | 34,117 | **首选采用** = 泰州市统计局首页 per 622 §0.2 候选清单 #1 |
| `https://tjj.suqian.gov.cn/` | — | — | fallback #1 未触发（首选一次成功）|
| `https://tjj.zhenjiang.gov.cn/` | — | — | fallback #2 未触发（首选一次成功；已用 per 621）|

**采用 = `tjj.taizhou.gov.cn` 泰州市统计局首页**（首选 per 622 tasking §0.2 verbatim 候选清单 #1；34,117 bytes；HTTP 200；SHA-256 = `55863f6553cdd13f18a26d4fb3d3b5b52c581ba3db41290c172c24a2c6d47307`）

**零 `--confirm-*` 字面** ✓
**零用户动作** ✓
**零用户裁定** ✓
**执行端零爬网公网（非政府域）** ✓（仅 tjj.taizhou.gov.cn 政府/统计局域；用户授权 #1 仍生效无需二次授权 per 622 §0.2 + 621 audit PASS precedent + 2026-08-29 治理铁律）

## §2. (B) 江苏样本地市第九刀 SHA-locked 落 `data/seed_archives/`

**执行**:
- `cp /tmp/622_taizhou.html data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html`
- 二次 SHA-256 验证 = `55863f6553cdd13f18a26d4fb3d3b5b52c581ba3db41290c172c24a2c6d47307` ✓
- bytes = 34,117（≥ 1 KB ✓）
- `source_registry/registry.csv` +1 行（line count 15 → 16）：
  - 新行 = `tjj.taizhou.gov.cn,泰州市统计局,MUNICIPAL_BULLETIN,...,data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html,55863f6553cdd13f18a26d4fb3d3b5b52c581ba3db41290c172c24a2c6d47307,34117,S0,...`
  - 18 列 schema 兼容既有 15 行（11 既有 + 617 盐城 + 619 扬州 + 621 镇江 + 622 泰州）
- 既有 11 行 SHA `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变（per 621 §5 EXISTING 11 ROWS IDENTICAL TO HEAD diff 验证）
...
**⚠ disclosure #1**: source_registry/registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期 per 622 §0.2 ⚠ disclosure + 606/607/608/609/610/611/612/613/614/616/617/618/619/620/621 precedent file-based role_count 守门不增计数）
...
- source_document 行新增 `doc_kind='OCR_SCAN'` + `source_sha256='55863f6553cdd...'` + `archive_path='data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html'`
...
## §5. (E) docs/45 §6.2 O1 status append（per 622 · 2026-08-30）

**落地**:
- docs/45 line 568+ append `> ⚠ **docs/45 §6.2 O1 status append**（per 622 · 2026-08-30）：O1 §5.2.x 江苏样本第九刀（地市样本第八刀）已落地（`55863f6553cd` per source_registry/registry.csv +1 行；tjj.taizhou.gov.cn 泰州市统计局首页 34,117 bytes per 622 §0.2 候选清单 #1 首选采用；执行端自取预 vetted 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；paddle-ocr e2e 在 .venv-paddle 隔离 venv 内接通 + HTML 路径走 docs/53 §5 connector 模式 + source_document + lineage JSONB mock writer 9 字段完整 + migration 001-013 零触碰 + 既有 11 行 SHA 零漂移）；江苏样本链路 9/15 节点；后续江苏样本刀待续接。docs 房规 NOT-IN-MANIFEST。`
...
## §7. (G) manifest bump K=4 → 992 → 996

**触发**: (A)(B)(C)(D)(E)(F) 全部 PASS

**落地**:
- `scripts/_knife622_manifest_bump.py` NEW spike_helper +1
- 622 audit PASS `622-stage0-architect-s621-o1-§5.2.x-real-sha-locked-江苏样本-地市第八刀-tasking-20260830-audit-PASS-20260830.md` (审计 OF 621 tasking = 江苏样本地市第八刀) 入库随 622 commit (per docs 房规「审计文件不单独 commit 随下一刀入库」= 622 audit 是 622 commit 的随附) NEW documentation +1
- 622 receipt（本文件）NEW documentation +1
- 江苏样本地市第九刀 HTML `data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html` 34,117 bytes / sha `55863f6553cdd...` NEW spike_sample_or_truth +1
- source_registry/registry.csv REFRESH（file-based role_count 守门不增计数 per 606/607/608/609/610/611/612/613/614/616/617/618/619/620/621 precedent；+1 行 bytes 总数变化是预期 per ⚠ disclosure #1；既有 11 行 SHA 不变）
...
- 618 receipt / 619 receipt / 620 receipt / 621 receipt 仅 narrative 措辞包裹形式（不动）
...
- 13 既有受保护文件 + source_registry/registry.csv +1 行（既有 11 行 SHA 不变）+ 江苏样本地市第九刀 HTML spike_sample_or_truth + 622 audit PASS 入库随 622 commit + 622 receipt 自身 = 14 受保护

**INVARIANT**: 996 == 996 == 996 ✓ (per scripts/_knife622_manifest_bump.py 实跑断言)

## §8. (H) 622 receipt 写回执（本文件）
...
- `extracts/` dir 不变 ✓
- `registry.csv` 既有 11 行 sha `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变（622 +1 行 bytes 总数变化是预期 per ⚠ disclosure #1）✓
...
- `data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html` 34,117 bytes / sha `55863f6553cdd...` NEW spike_sample_or_truth

**31+ 红线 100% 兑现** (per 622 §0.3 + 2026-08-29 治理铁律):
- ❌ Stage 0/Gate 1/2 PASS / O1 PASS / O3 PASS 零重新宣告 ✓
- ❌ 2020-2025 batch work 零批量（本刀仅 1 个江苏样本地市第九刀 HTML 样本）✓
- ❌ 公网爬网（非政府/统计局）零（本刀零网络访问公网；仅 tjj.taizhou.gov.cn 政府源）✓
...
- ❌ 重新宣告 O3 整体 CLOSED 零（per 588 + 590 + 597 + 598 + 599 + 600 + 601 + 602 + 603 + 604 + 605 + 606 + 607 + 608 + 609 + 610 + 611 + 612 + 613 + 614 + 615 + 616 + 617 + 618 + 619 + 620 + 621 二十六重声明；622 不二次宣告）✓
- ❌ 重新宣告 O1 整体收口 零（O1 整体仍 WAITING_FILE per docs/47 §3.1；622 仅江苏样本地市第九刀 SHA-locked 不构成 O1 整体收口）✓
...
- ❌ 修改 source_registry/registry.csv 既有 11 行 零（既有 11 行 SHA `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变；622 仅 +1 行 bytes 总数变化是预期 per ⚠ disclosure #1）✓
...
- ❌ 修改 620 audit PASS / 621 audit PASS (= 622 audit PASS) 文件 零（架构师自签；执行端零修改；仅随 622 commit 入库 per docs 房规；本刀仅入 manifest 不改内容）✓
- ❌ 修改 622 audit (OF 622 tasking) 文件 零（架构师尚未自签；执行端零创建零修改；跟 623 commit 入库 per docs 房规）✓
- ❌ 修改 617 receipt / 618 receipt / 619 receipt / 620 receipt / 621 receipt 实质内容 零 ✓
...
- ❌ 真实 PDF 上传（非 seed_archives/）零（仅 `data/seed_archives/jiangsu_taizhou_tjj_gov_cn_20260830.html` 落）✓
...
- ❌ 用户授权 #1 二次申请 零（用户授权 #1 仍生效无需二次申请 per 622 §0.2 verbatim + 2026-08-29 治理铁律）✓

**⚠ disclosures (1 项 ACCEPTED per 622 tasking §0.2)**:

**⚠ #1 (source_registry/registry.csv +1 行)**: per 622 tasking §0.2 ⚠ disclosure #1 — registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数）✓
...
- **下一刀候选** (per 622 tasking §5 + 622 audit §7 + 621 receipt §9 + 620 audit §7 优先级 2 verbatim):
  - **623 tasking** 候选 #1：622 receipt 审计刀（per 583/585/587/589/591/593/594/595/596/597/598/599/600/601/602/603/604/605/606/607/608/609/610/611/612/613/614/615/616/617/618/619/620/621/622 audit precedent）
  - **623 tasking** 候选 #2：O1 §5.2.x 江苏样本第十刀（地市样本第九刀；剩余江苏地市 = 宿迁地市统计局公开源；接续 605 + 606 + 608 + 610 + 612 + 617 + 619 + 621 + 622 江苏样本链路 9/15 → 10/15）
  - **623 tasking** 候选 #3：O1 §5.2.x 江苏样本省样本第二刀（其它省统计局公开源：如浙江/广东/山东等省统计局公开源；接续 605 首批省样本链路）
  - **623 tasking** 候选 #4：其它治理推进刀 — 任一由架构师定夺 per 615 audit §7.1 优先级 3/4

- **O1 整体仍 WAITING_FILE**（per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；622 仅江苏样本地市第九刀 SHA-locked 不构成 O1 整体收口；待全部 5 省样本 + 10 地市样本均 SHA-locked 后另刀审议）
- **B 路（公开源自动获取 per docs/52）保持主路径**
- **A 路（用户投递 per docs/51）保留为 fallback 标注（不删除、不调用）**
- **O3 整体仍 CLOSED 候选**（per 588 + 590 + 597 + 598 + 599 + 600 + 601 + 602 + 603 + 604 + 605 + 606 + 607 + 608 + 609 + 610 + 611 + 612 + 613 + 614 + 615 + 616 + 617 + 618 + 619 + 620 + 621 二十六重声明；622 不二次宣告）
- **江苏样本链路进度**: 605 首批省样本（stats.gov.cn 江苏分省页面 1 节点）+ 606 首批地市样本（tjj.suzhou.gov.cn 苏州市统计局 1 节点）+ 608 第二批地市样本（tjj.nanjing.gov.cn 南京市统计局 1 节点）+ 610 第三批地市样本（tjj.changzhou.gov.cn 常州市统计局 1 节点）+ 612 第四批地市样本（tjj.nantong.gov.cn 南通市统计局 1 节点）+ 617 第六刀地市样本（tjj.yancheng.gov.cn 盐城市统计局 1 节点）+ 619 第七刀地市样本（tjj.yangzhou.gov.cn 扬州市统计局 1 节点）+ 621 第八刀地市样本（tjj.zhenjiang.gov.cn 镇江市统计局 1 节点）+ 622 第九刀地市样本（tjj.taizhou.gov.cn 泰州市统计局 1 节点）= 江苏样本链路 9 节点；目标 5 省 + 10 地市 = 15 节点；剩余 6 节点待续接
- **preferred candidate 首选采用**: 622 tasking §0.2 候选清单 #1 泰州市统计局首页 HTTP 200 34,117 bytes 首选一次成功，无 fallback 触发（其它 fallback #1-#2 仅作备援）

---

— End of `622-stage0-cc-o1-§5.2.x-real-sha-locked-江苏样本-地市第九刀-tasking-20260830-receipt.md` —
