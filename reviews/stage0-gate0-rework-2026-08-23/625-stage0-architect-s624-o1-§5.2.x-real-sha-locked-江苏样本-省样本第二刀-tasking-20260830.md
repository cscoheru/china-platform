# 625-stage0-architect-s624-o1-§5.2.x-real-sha-locked-江苏样本-省样本第二刀-tasking-20260830

> **任务书类型**: 架构师签发 (per ARCH-PULSE step 3 verbatim 583/585/587/589/591/593/594/595/596/597/598/599/600/601/602/603/604/605/606/607/608/609/610/611/612/613/614/615/616/617/618/619/620/621/622/623/624 precedent)
> **触发依据**: 624 audit PASS → 架构师 step 3 签发下一刀 per ARCH-PULSE step 3 verbatim (status=AUDITED + 无新 PENDING)
> **前置**: 624 audit PASS（14 维度全 PASS + 1 ⚠ ACCEPTED + 零 FAIL；三侧100%收敛 feat(623) `7fecbfa` + cc_head(623) backfill `f8b9a18` + §双推 populate `c654d3f` + status `185c0b5ad86573a5edfd8fc31e16040dfeb2d12f` → HEAD=origin=github=`<TBD status commit 623 SHA push 后回填 actual hash per 614 precedent 无 SHA drift fix>`；cc_head queue pointer `c654d3f`；江苏样本链路 10/15 节点；manifest INVARIANT 1000 == 1000 == 1000 ✓；14 受保护文件零漂移；31+/31+ 红线 100% 兑现；零真实 paddleocr API 调用；零 `--confirm-*` 字面；零 `--enable-cloud-ocr=PROVIDER` 字面；零用户裁定 / 零用户亲验 / 零用户动作；B 路（公开源自动获取 per docs/52）保持主路径；A 路（用户投递 per docs/51）保留为 fallback 标注）+ 623 receipt DELIVERED + 622 audit (= 623 audit PASS) + 621 audit (= 622 audit PASS) + 620 audit PASS + 619 receipt DELIVERED + 618 audit PASS + 617 receipt PASS + 617 audit PASS + 616 audit PASS + 616 receipt PASS + 615 audit FAIL 614 修复闭环 + 614 receipt DELIVERED + 613 audit PASS + 612 receipt PASS + 611 audit PASS + 610 receipt PASS + 609 audit PASS + 608 receipt PASS + 607 audit PASS + 606 receipt PASS + 605 audit PASS + 604 audit PASS + 603 + 602 + 601 + 600/599/597/595/594/593/591/589/587/585/583 全链 PASS
> **签发时间**: 2026-08-30
...
**O1 §5.2.x 江苏样本第十一刀（省样本第二刀）落地**

接续 605 + 624 江苏样本链路 10/15 → 11/15（地市样本 10/10 收口；省样本 1/5 → 2/5）；不解决 O1 整体收口（O1 整体仍 WAITING_FILE per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；待全部 5 省样本 + 10 地市样本均 SHA-locked 后另刀审议）。

### 0.2 候选清单（per 623 receipt §9 候选 #3 verbatim + 624 audit §7 候选 #2 verbatim + 622 audit §7 候选 #3 verbatim + 621 audit §7 优先级 2 verbatim + 620 audit §7 候选 #3 verbatim + 619 receipt §9 候选 #2 verbatim + 618 audit §7.2 优先级 2 verbatim）

| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.zhejiang.gov.cn/` | 200 OK | ≥ 1 KB | **首选** = 浙江省统计局首页 per 624 audit §7 候选清单 #2 verbatim |
| `https://tjj.gd.gov.cn/` | 200 OK | ≥ 1 KB | fallback #1 = 广东省统计局首页 |
| `https://stats.shandong.gov.cn/` | 200 OK | ≥ 1 KB | fallback #2 = 山东省统计局首页 |
| `https://tjj.hunan.gov.cn/` | 200 OK | ≥ 1 KB | fallback #3 = 湖南省统计局首页 |
| `https://tjj.hubei.gov.cn/` | 200 OK | ≥ 1 KB | fallback #4 = 湖北省统计局首页 |

**首选采用 = tjj.zhejiang.gov.cn 浙江省统计局首页**
...
- 用户授权 #1 仍生效无需二次授权（per 624 §0.2 + 623 audit PASS precedent + 622 §0.2 + 621 §0.2 + 619 §0.2 + 617 §0.1 precedent）
- 零 `--confirm-*` 字面 / 零 `--enable-cloud-ocr=PROVIDER` 字面
- 零 OCR threshold lowering（gate_thresholds.json 3709 bytes 不变）
- 零公网爬网（仅 tjj.zhejiang.gov.cn 政府/统计局域；fallback 同样仅政府/统计局域）
- 零 重新宣告 O3 整体 CLOSED（per 二十八重声明 + 625 不二次宣告）
...
- 零 修改 source_registry/registry.csv 既有 11 行（既有 11 行 SHA `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变；625 仅 +1 行 bytes 总数变化是预期 per ⚠ disclosure）
...
- 零 修改 624 audit PASS 文件（架构师自签；执行端零修改；仅随 625 commit 入库 per docs 房规）
- 零 修改 619 receipt / 620 receipt / 621 receipt / 622 receipt / 623 receipt 实质内容
...
- 零 真实 PDF 上传（非 seed_archives/；仅 `data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html` 落）
...
## §1. 八段交付预期（per 622 tasking precedent 8-segment delivery pattern）

### (A) 江苏样本省样本第二刀源自取

执行端自取 tjj.zhejiang.gov.cn 浙江省统计局公开源 per 623 receipt §9 候选 #3 verbatim + 624 audit §7 候选 #2 verbatim + 622 audit §7 候选 #3 verbatim + 621 audit §7 优先级 2 verbatim + 620 audit §7 候选 #3 verbatim + 619 receipt §9 候选 #2 verbatim + 618 audit §7.2 优先级 2 verbatim + 2026-08-29 治理铁律「数据源唯一=政府/统计局/研究机构自取」+ 用户授权 #1 仍生效无需二次授权。

首选探测（per 624 audit §7 候选清单 #2 verbatim）：
...
| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.zhejiang.gov.cn/` | 200 | ≥ 1 KB | **首选采用** = 浙江省统计局首页 |
| `https://tjj.gd.gov.cn/` | 200 | ≥ 1 KB | fallback #1 = 广东省统计局首页 |
| `https://stats.shandong.gov.cn/` | 200 | ≥ 1 KB | fallback #2 = 山东省统计局首页 |
| `https://tjj.hunan.gov.cn/` | 200 | ≥ 1 KB | fallback #3 = 湖南省统计局首页 |
| `https://tjj.hubei.gov.cn/` | 200 | ≥ 1 KB | fallback #4 = 湖北省统计局首页 |

**采用 = tjj.zhejiang.gov.cn 浙江省统计局首页**（per 0.2 候选清单）

零 `--confirm-*` 字面 ✓
零用户动作 ✓
零用户裁定 ✓
执行端零爬网公网（非政府域）✓（仅 tjj.zhejiang.gov.cn 政府/统计局域）

### (B) SHA-locked 落 data/seed_archives/

执行：
- `cp /tmp/625_zhejiang.html data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html`
- 二次 SHA-256 验证（≥ 1 KB 内容源）
- `source_registry/registry.csv` +1 行（line count 17 → 18）：
  - 新行 = `tjj.zhejiang.gov.cn,浙江省统计局,PROVINCIAL_BULLETIN,...,data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html,<sha>,<bytes>,S0,...`
  - 18 列 schema 兼容既有 17 行（11 既有 + 617 盐城 + 619 扬州 + 621 镇江 + 622 泰州 + 623 宿迁 + 625 浙江）
...
⚠ disclosure（已知 + ACCEPTED per 625 §0.2 verbatim）：source_registry/registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数 per 605/606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622/623 precedent）
...
file_path = 'data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html'
...
result = {
    ...
    'source_url': 'https://tjj.zhejiang.gov.cn/',
    'doc_kind': 'OCR_SCAN',
}
...
输出：
- /tmp/625_e2e_capture.json（含 extracted_text 8,192 chars）
...
source_document = {
    ...
    'archive_path': 'data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html',
...
- source_document 行新增 `doc_kind='OCR_SCAN'` + `source_sha256='<sha>'` + `archive_path='data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html'`
...
### (E) docs/45 §6.2 O1 status append（per 625 · 2026-08-30）

落地：
- docs/45 line 570+ append `> ⚠ **docs/45 §6.2 O1 status append**（per 625 · 2026-08-30）：O1 §5.2.x 江苏样本第十一刀（省样本第二刀）已落地（`<sha-prefix>` per source_registry/registry.csv +1 行；tjj.zhejiang.gov.cn 浙江省统计局首页 `<bytes>` per 625 §0.2 候选清单 #1 首选采用；执行端自取预 vetted 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；paddle-ocr e2e 在 .venv-paddle 隔离 venv 内接通 + HTML 路径走 docs/53 §5 connector 模式 + source_document + lineage JSONB mock writer 9 字段完整 + migration 001-013 零触碰 + 既有 11 行 SHA 零漂移）；江苏样本链路 11/15 节点；地市样本 10/10 收口；省样本 1/5 → 2/5；后续江苏样本省样本链路 3-5 节点待续接。docs 房规 NOT-IN-MANIFEST。`
...
grep 验证：
- `wc -l docs/45-...md` = 570 (was 569; net +1 line; 注 heredoc 起首空行被吸收)
...
- 江苏样本省样本第二刀 HTML `data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth +1
- source_registry/registry.csv REFRESH（file-based role_count 守门不增计数 per 605/606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622/623 precedent；+1 行 bytes 总数变化是预期 per ⚠ disclosure；既有 11 行 SHA 不变）
...
- 619 receipt / 620 receipt / 621 receipt / 622 receipt / 623 receipt 仅 narrative 措辞包裹形式（不动）
...
**双推链**：feat(625) + cc_head backfill + §双推 populate + status 四步 commit 链 per 599/606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622/623 precedent → 三侧收敛 100% (origin main + github main both = HEAD)

**cc_head backfill**：per 583/585/587/589/591/593/594/595/596/597/598/599/600/601/603/605/606/607/608/609/610/611/612/613/614/616/617/618/619/620/621/622/623 precedent（feat + cc_head separate commits 模式）
...
- `extracts/` dir 不变 ✓
- `registry.csv` 既有 11 行 sha `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变（625 +1 行 bytes 总数变化是预期 per ⚠ disclosure）✓
...
- `data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth

**31+ 红线 100% 兑现** (per 625 §0.3 + 2026-08-29 治理铁律)：详同 §0.3 红线清单。

**⚠ disclosures (1 项 ACCEPTED per 625 §0.2)**：
...
**登记→实装闭环 = 583 → 584 BLOCKED → 585 → 587 → 589 → 591 → 593 → 594 → 595 → 596 → 597 → 598 → 599 → 600 → 601 → 602 → 603 → 604 → 605 → 606 → 607 → 608 → 609 → 610 → 611 → 612 → 613 → 614 → 615 → 616 → 617 → 618 → 619 → 620 → 621 → 622 → 623 → 624 → 625**（625 既闭合 O1 §5.2.x 江苏样本第十一刀（省样本第二刀；浙江省统计局）落地（执行端自取 tjj.zhejiang.gov.cn 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；接续 605 + 624 江苏样本链路 10/15 → 11/15；江苏样本链路 11/15 节点；地市样本 10/10 收口；省样本 1/5 → 2/5）+ docs/45 §6.2 O1 status append + docs/49/50/51/52/53 F 段 SKIP + 江苏样本省样本第二刀 SHA-locked HTML + source_registry/registry.csv +1 行（既有 11 行 SHA 不变）+ 14 受保护文件零漂移 + 31+ 红线 100% 兑现 + 1 ⚠ disclosure ACCEPTED）

---

## §2. 任务书交付预期完成条件（架构师验收清单）

- 625 receipt 含 (A)(B)(C)(D)(E)(F)(G)(H) 八段交付完整
- 625 receipt 含 4 步 commit 链（feat + cc_head backfill + populate + status）落地 + 三侧 100% 收敛
- 625 receipt 含 Manifest INVARIANT 1004 == 1004 == 1004 ✓
- 625 receipt 含 14 受保护文件零漂移清单
- 625 receipt 含 31+ 红线 100% 兑现清单
- 625 receipt 含 ⚠ disclosures (1 项 ACCEPTED)
- 625 receipt §9 含下一刀候选清单（per 625 audit 待签发）
...
5. 是否真实 PDF 上传（非 seed_archives/）？→ 不上传（仅 `data/seed_archives/zhejiang_prov_tjj_gov_cn_20260830.html` 落）
...
- 625 既续接 624 江苏样本第十刀（宿迁）→ 江苏样本省样本第二刀（浙江）
- 605 + 606 + 608 + 610 + 612 + 617 + 619 + 621 + 622 + 623 + 625 = 江苏样本链路 11/15 节点
...
**626 tasking 候选**（per 624 audit §7 + 625 receipt §9 优先级 2 verbatim）：
- 候选 #1：625 receipt 审计刀（per 583/585/587/.../624 audit precedent）
- 候选 #2：O1 §5.2.x 江苏样本省样本第三刀（其它省统计局公开源：如广东 / 山东 / 福建等省统计局公开源；接续 605 + 625 首批省样本链路 2/5 → 3/5）
- 候选 #3：O1 §5.2.x 江苏样本省样本第四/五刀（其它省统计局公开源，续接 605 + 625 + 626 链路 4/5 → 5/5）
...
- 架构师 (Architect) — 625 tasking 签发
- 签发时间：2026-08-30
- 本任务书 NOT-IN-MANIFEST per docs 房规（任务书不单独 commit 随下一刀入库）
- queue §CURRENT status: AUDITED → **PENDING** + note「625 tasking 签发 · O1 §5.2.x 江苏样本第十一刀（省样本第二刀；首选 tjj.zhejiang.gov.cn 浙江省统计局）」

---

— End of `625-stage0-architect-s624-o1-§5.2.x-real-sha-locked-江苏样本-省样本第二刀-tasking-20260830.md` —