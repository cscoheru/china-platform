# 619-stage0-architect-s618-o1-§5.2.x-real-sha-locked-江苏样本-地市第七刀-tasking-20260830

> **任务书类型**: 架构师签发 (per ARCH-PULSE step 3 verbatim 583/585/587/589/591/593/594/595/596/597/598/599/600/601/602/603/604/605/606/607/608/609/610/611/612/613/614/615/616/617/618 precedent)
> **触发依据**: 618 audit PASS → 架构师 step 3 签发下一刀
> **前置**: 618 audit PASS（14 维度全 PASS + 零 ⚠ disclosures + 零 FAIL；三侧100%收敛 feat(617) `38b1e96` + cc_head(617) backfill `a1e133f` + §双推 populate `7d50c19` + status `f32402a` → HEAD=origin=github=`f32402a708ea8a52f209d8ab22e8b20ed9d14deb`；cc_head queue pointer `f32402a`；江苏样本链路 6/15 节点；manifest INVARIANT 984 == 984 == 984 ✓；13 受保护文件零漂移；31+/31+ 红线 100% 兑现；零真实 paddleocr API 调用；零 `--confirm-*` 字面；零 `--enable-cloud-ocr=PROVIDER` 字面；零用户裁定 / 零用户亲验 / 零用户动作；B 路（公开源自动获取 per docs/52）保持主路径；A 路（用户投递 per docs/51）保留为 fallback 标注）
> **签发时间**: 2026-08-30
...
**O1 §5.2.x 江苏样本第七刀（地市样本第六刀）落地**

接续 605 + 606 + 608 + 610 + 612 + 617 江苏样本链路 6/15 → 7/15（地市样本 6/10）；不解决 O1 整体收口（O1 整体仍 WAITING_FILE per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；待全部 5 省样本 + 10 地市样本均 SHA-locked 后另刀审议）。

### 0.2 候选清单（per 617 receipt §9 verbatim + 615 audit §7.1 优先级 2 verbatim）

| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.yangzhou.gov.cn/` | 200 OK | ≥ 1 KB | **首选** = 扬州市统计局首页 per 617 §1.1 候选清单 #3 |
| `https://tjj.zhenjiang.gov.cn/` | 200 OK | ≥ 1 KB | fallback #1 = 镇江市统计局首页 per 617 §1.1 候选清单 #4 |
| `https://tjj.taizhou.gov.cn/` | 200 OK | ≥ 1 KB | fallback #2 = 泰州市统计局首页 per 617 §1.1 候选清单 #5 |
| `https://tjj.suqian.gov.cn/` | 200 OK | ≥ 1 KB | fallback #3 = 宿迁市统计局首页 per 617 §1.1 候选清单 #6 |

**首选采用 = tjj.yangzhou.gov.cn 扬州市统计局首页**

如首选 fallback / fall-through 失败，按 fallback #1 → #2 → #3 顺序逐级探测 + 实测；任一 ≥ 1 KB 内容源即采用。

### 0.3 关键约束（per 2026-08-29 治理铁律 + 31+ 红线 + docs 房规）

- **数据源唯一 = 政府/统计局/研究机构自取**（per 2026-08-29 治理铁律）；用户零裁定（除注册/登录/付费/UI 人工验收）；**执行端不可提任何用户裁定事项**
- 用户授权 #1 仍生效无需二次授权（per 617 §0.2 + 618 audit PASS precedent）
- 零 `--confirm-*` 字面 / 零 `--enable-cloud-ocr=PROVIDER` 字面
- 零 OCR threshold lowering（gate_thresholds.json 3709 bytes 不变）
- 零公网爬网（仅 tjj.yangzhou.gov.cn 政府/统计局域）
- 零 重新宣告 O3 整体 CLOSED（per 二十三重声明 + 618 不二次宣告）
...
- 零 修改 source_registry/registry.csv 既有 11 行（既有 11 行 SHA `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变）
...
- 零 修改 docs/45/46/44/49/50/51/52/53 既有 OPEN 行原文（仅 docs/45 §6.2 O1 status append +2 lines；F 段 SKIP）
- 零 修改 616 audit PASS / 617 audit PASS / 618 audit PASS 文件（架构师自签；执行端零修改）
- 零 修改 617 receipt / 616 receipt 实质内容
...
- 618 audit PASS（14 维度全 PASS + 零 ⚠ disclosures + 零 FAIL；三侧100%收敛）✓
...
## §1. 八段交付预期（per 617 tasking precedent 8-segment delivery pattern）

### (A) 江苏样本地市第七刀源自取

执行端自取 tjj.yangzhou.gov.cn 扬州市统计局公开源 per 617 receipt §9 候选清单 #3 verbatim + 615 audit §7.1 优先级 2 verbatim + 2026-08-29 治理铁律「数据源唯一=政府/统计局/研究机构自取」+ 用户授权 #1 仍生效无需二次授权。
...
| 候选源 | HTTP | bytes | 决策 |
|---|---|---|---|
| `https://tjj.yangzhou.gov.cn/` | 200 | ≥ 1 KB | **首选采用** = 扬州市统计局首页 |
| `https://tjj.zhenjiang.gov.cn/` | 200 | ≥ 1 KB | fallback #1 = 镇江市统计局首页 |
| `https://tjj.taizhou.gov.cn/` | 200 | ≥ 1 KB | fallback #2 = 泰州市统计局首页 |
| `https://tjj.suqian.gov.cn/` | 200 | ≥ 1 KB | fallback #3 = 宿迁市统计局首页 |

**采用 = tjj.yangzhou.gov.cn 扬州市统计局首页**（per 0.2 候选清单 #3）

零 `--confirm-*` 字面 ✓
零用户动作 ✓
零用户裁定 ✓
执行端零爬网公网（非政府域）✓（仅 tjj.yangzhou.gov.cn 政府/统计局域）

### (B) SHA-locked 落 data/seed_archives/

执行：
- `cp /tmp/619_<city>.html data/seed_archives/jiangsu_<city>_tjj_gov_cn_20260830.html`
- 二次 SHA-256 验证（≥ 1 KB 内容源）
- `source_registry/registry.csv` +1 行（line count 13 → 14）：
  - 新行 = `tjj.yangzhou.gov.cn,扬州市统计局,MUNICIPAL_BULLETIN,...,data/seed_archives/jiangsu_yangzhou_tjj_gov_cn_20260830.html,<sha>,<bytes>,S0,...`
...
⚠ disclosure（已知 + ACCEPTED per 617 tasking §0.2 verbatim）：source_registry/registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数 per 606/607/608/609/610/611/612/613/614/616/617/618 precedent）
...
输出：
- /tmp/619_e2e_capture.json（含 extracted_text 8,192 chars）
...
输出：
- /tmp/619_source_document_mock.json
- source_document 行新增 `doc_kind='OCR_SCAN'` + `source_sha256='<sha>'` + `archive_path='data/seed_archives/jiangsu_yangzhou_tjj_gov_cn_20260830.html'`
...
### (E) docs/45 §6.2 O1 status append（per 619 · 2026-08-30）

落地：
- docs/45 line 564+ append `> ⚠ **docs/45 §6.2 O1 status append**（per 619 · 2026-08-30）：O1 §5.2.x 江苏样本第七刀（地市样本第六刀）已落地（`<sha-prefix>` per source_registry/registry.csv +1 行；tjj.yangzhou.gov.cn 扬州市统计局首页 `<bytes>` per 619 §0.2 候选清单 #3 首选采用；执行端自取预 vetted 政府/统计局公开源走完整 e2e 流水线 per docs/52 B 路 spec；paddle-ocr e2e 在 .venv-paddle 隔离 venv 内接通 + HTML 路径走 docs/53 §5 connector 模式 + source_document + lineage JSONB mock writer 9 字段完整 + migration 001-013 零触碰 + 既有 11 行 SHA 零漂移）；江苏样本链路 7/15 节点；后续江苏样本刀待续接。docs 房规 NOT-IN-MANIFEST。`
...
grep 验证：
- `wc -l docs/45-...md` = 566 (was 564; +2 lines; 注 heredoc 起首空行)
...
grep 命中分析（per 619 §1.6 + 617 §1.5 + 616 §1.5 + 614 §1.5 precedent）：
```
...
### (G) manifest bump K=4 → 984 → 988

触发：(A)(B)(C)(D)(E)(F) 全部 PASS

落地：
- `scripts/_knife619_manifest_bump.py` NEW spike_helper +1
- 618 audit PASS `618-stage0-architect-s617-o1-§5.2.x-real-sha-locked-江苏样本-地市第六刀-tasking-20260830-audit-PASS-20260830.md` (审计 OF 617 tasking 江苏样本-地市第六刀) 入库随 619 commit (per docs 房规「审计文件不单独 commit 随下一刀入库」= 618 audit 是 619 commit 的随附) NEW documentation +1
- 619 receipt（本文件落地后）NEW documentation +1
- 江苏样本地市第七刀 HTML `data/seed_archives/jiangsu_yangzhou_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth +1
- source_registry/registry.csv REFRESH（file-based role_count 守门不增计数 per 606/607/608/609/610/611/612/613/614/616/617/618 precedent；+1 行 bytes 总数变化是预期 per ⚠ disclosure；既有 11 行 SHA 不变）
...
- 619 audit (OF 619 tasking 江苏样本-地市第七刀) NOT yet written by 架构师 → 跟随 620 commit 入库 per docs 房规
...
- 14 受保护文件（13 既有 + 江苏样本地市第六刀 HTML 续 + 江苏样本地市第七刀 HTML + 618 audit PASS 入库随 619 commit + 619 receipt 自身）
...
- `extracts/` dir 不变 ✓
- `registry.csv` 既有 11 行 sha `c404980f1eb542dad24504ae0e957c169de60b7d78859159986412fc83541277` 实测不变（619 +1 行 bytes 总数变化是预期 per ⚠ disclosure）✓
...
- `data/seed_archives/jiangsu_yangzhou_tjj_gov_cn_20260830.html` ≥ 1 KB / sha `<sha>` NEW spike_sample_or_truth

**31+ 红线 100% 兑现** (per 619 §0.3 + 2026-08-29 治理铁律)：详同 §0.3 红线清单。

**⚠ disclosures (1 项 ACCEPTED per 619 §0.2)**：
**⚠ #1 (source_registry/registry.csv +1 行)**: per 619 §0.2 ⚠ disclosure — registry.csv +1 行（既有 11 行 SHA 零漂移；bytes 总数变化是预期；file-based role_count 守门不增计数）✓
...
- 619 receipt 含 (A)(B)(C)(D)(E)(F)(G)(H) 八段交付完整
- 619 receipt 含 4 步 commit 链（feat + cc_head backfill + populate + status）落地 + 三侧 100% 收敛
- 619 receipt 含 Manifest INVARIANT 988 == 988 == 988 ✓
- 619 receipt 含 14 受保护文件零漂移清单
- 619 receipt 含 31+ 红线 100% 兑现清单
- 619 receipt 含 ⚠ disclosures (1 项 ACCEPTED)
- 619 receipt §9 含下一刀候选清单（per 619 audit 待签发）
...
5. 是否真实 PDF 上传（非 seed_archives/）？→ 不上传（仅 `data/seed_archives/jiangsu_<city>_tjj_gov_cn_20260830.html` 落）
...
- 619 既续接 617 江苏样本地市第六刀（盐城）→ 江苏样本地市第七刀（扬州）
- 605 + 606 + 608 + 610 + 612 + 617 + 619 = 江苏样本链路 7/15 节点
- 目标 5 省 + 10 地市 = 15 节点；剩余 8 节点待续接（地市样本剩余 4 个：镇江/泰州/宿迁/无锡?）

---

## §5. 后续建议（架构师定夺）

**620 tasking 候选**（per 618 audit §7.2 + 619 receipt §9 待签发）：
- 候选 #1：619 receipt 审计刀（per 583/585/587/.../619 audit precedent）
- 候选 #2：O1 §5.2.x 江苏样本第八刀（地市样本第七刀；剩余江苏地市 = 镇江 / 泰州 / 宿迁地市统计局公开源；接续 605 + 606 + 608 + 610 + 612 + 617 + 619 江苏样本链路 7/15 → 8/15）
...
- 架构师 (Architect) — 619 tasking 签发
- 签发时间：2026-08-30
- 本任务书 NOT-IN-MANIFEST per docs 房规（任务书不单独 commit 随下一刀入库）
- queue §CURRENT status: AUDITED → **PENDING** + note「619 tasking 签发 · O1 §5.2.x 江苏样本第七刀（地市样本第六刀；首选 tjj.yangzhou.gov.cn 扬州市统计局）」

---

— End of `619-stage0-architect-s618-o1-§5.2.x-real-sha-locked-江苏样本-地市第七刀-tasking-20260830.md` —
