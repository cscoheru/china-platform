...
| 5.2.4 | paddle-ocr 本地依赖 + Dockerfile layer | ✅ **CLOSED per 597（2026-08-29）** — paddle-ocr MOCK only 路径 + 端到端 pytest PASS + 真实 PDF e2e 验证 + 584 docs sync 收口（per `597-stage0-architect-s596-584-reack-impl-tasking-20260829.md` + `597-stage0-cc-584-reack-impl-tasking-20260829-receipt.md` + `596-stage0-architect-s595-584-reack-ready-tasking-20260829.md` + `596-stage0-cc-584-reack-ready-tasking-20260829-receipt.md`）；584 BLOCKER 5 → 0 全闭环收口（594 评估 = 5 → 1 仅 P2 保留；595 解除刀落地 = 5 → 0 = P2 ✅ Colima + P3 ✅ Dockerfile + P4 ✅ requirements-paddle.txt + P1 ✅ Python 3.11 wheel + 用户裁定 auto-accept per 2026-08-28 夜起常设授权；596 paddle-ocr deps 实际引入 + Dockerfile build/run 验证 = 准备就绪；597 = 584 §5.2.4 完整实施闭合；详见 `597-stage0-architect-s596-584-reack-ready-tasking-20260829-audit-PASS-20260829.md` PASS audit）；per `583-stage0-cc-o3-impl-validate-api-doc-kind-receipt-20260828` + `585-stage0-cc-o3-impl-e2e-pytest-tasking-20260829-receipt` + `587-stage0-cc-o3-impl-real-pdf-e2e-tasking-20260829-receipt` + `595-stage0-cc-blocker-relief-dockerfile-deps-tasking-20260829-receipt` + `596-stage0-cc-584-reack-ready-tasking-20260829-receipt` + 597 receipt；零真实 paddleocr API 调用 + 零真实 PDF 上传 + 零触真实 DB + 零 paddlepaddle 安装到 system site-packages（仅 `.venv-paddle` venv）+ 零 .venv-dbt 污染 + 零 requirements-dbt.txt 修改 + 零 docs/X 修改（除 584 stale 行 selective refresh）|

> [superseded per 597（2026-08-29）· per 2026-08-29 治理铁律：数据源唯一=政府/统计局/研究机构自取；执行端自取预 vetted 源走完整 e2e 流水线；零 `--confirm-*` 字面；584 BLOCKED-DEFERRED 状态行已转为 CLOSED per 597；584 重 ACK 触发条件全部满足 = Python 3.11 wheel ✅ + Docker daemon ✅ + Dockerfile ✅ + paddle-ocr deps 实际引入 ✅ + 用户裁定 auto-accept per 2026-08-28 夜起常设授权 ✅ + paddle-ocr MOCK only 与 deps 解耦先例 ✅ per 585 §0.2；本 docs/49 §5.2 row 5.2.4 原文「BLOCKED-DEFERRED per 584（2026-08-29）· Path C」表述 supersede 为 CLOSED per 597（per 「BLOCKED-DEFERRED → CLOSED 状态行的 BLOCKED-DEFERRED 表述 supersede」红线例外），4 BLOCKER 历史描述保留为治理教训，supersede 标注与原文共存（per 「不删既有 OPEN 行」红线 + 「不删旧 row」教训模式 + 589 + 591 + 593 + 595 平行模式）；**O3 §5.2.4 paddle-ocr 本地依赖 + Dockerfile layer = CLOSED per 597**]

> [superseded per 599（2026-08-29）· **598 audit 落**（per `598-stage0-architect-s597-584-impl-audit-PASS-20260829.md` PASS audit；584 §5.2.4 paddle-ocr 引擎依赖实施刀 = O3 §5.2.4 CLOSED 候选 per 588 PASS + 590 PASS + 597 三重声明 + 598 audit PASS 四重声明；584 BLOCKED-DEFERRED → CLOSED per 597 状态行已为审计签发；docs/49 §5.2 row 5.2.4 「CLOSED per 597（2026-08-29）」追加 598 audit 落标注 共存；不删 597 原文 = 「不删既有 OPEN 行」红线 + 「不删旧 row」教训模式 + 589 + 591 + 593 + 595 + 596 + 597 平行模式）；**O3 §5.2.4 paddle-ocr 本地依赖 + Dockerfile layer = CLOSED 候选 per 588 PASS + 590 PASS + 597 三重声明 + 598 audit 落 四重声明**；B 路（公开源自动获取 per docs/52）保持主路径；O1 整体仍 WAITING_FILE per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；584 重 ACK 触发条件保留 = 用户裁定 + Python 3.12 wheel 可用 + Docker daemon 就绪 + 项目主 deps manifest 决策已定 + Dockerfile（非 current critical path）；后续 599 tasking = docs/52 B 路 spec 落定刀 + O1 §5.2.x 真实 SHA-locked 江苏样本刀（待 docs/52 B 路落定后另刀下发）— 任一由架构师定夺]
...
| 5.2.6 | 至少 1 个真实 PDF（用户裁定提供；非爬源）| ✅ **CLOSED per 587（2026-08-29）** — 执行端自取 S0 源 = `spikes/04-scanned-pdf/data/shaanxi_fiscal_regulation_flk.pdf` 全国人大常委会国家法律法规数据库陕西财政预算管理条例 4 页灰度扫描 PDF；SHA `f34b2e57ae08620cb6a6afb98b3983d805d53e3bae78b969795987a7ebe71488` = registry.csv 注册 SHA 验证一致（实测 1007943 bytes）+ 复制到 ALLOWED_PREFIXES[0] `/tmp/cegr_uploads/shaanxi_fiscal_regulation_flk.pdf` + sha256sum 验证零漂移 + `validate_ocr_input` ACCEPT + paddle-ocr MOCK only（4 页陕西财政预算管理条例 canned 文本；`patch.dict(sys.modules, ...)` + `instance_mock.ocr` 捕获路径参数；与 deps 引入解耦 per `584` BLOCKED-DEFERRED Path C）+ source_document mock writer 捕获 row dict（`doc_kind='OCR_SCAN'` + `language='zh-CN'` + `page_count=4` + `upload_user_id='executor_587'`）+ lineage JSONB 12 字段完整（`engine='paddle-ocr'` + `confidence=0.95` + `page_count=4` + `extracted_text='陕西省财政预算管理条例（具体内容详见 PDF 嵌入旧 OCR 文本层）'` + `is_demo=false` + `source_file_sha256='f34b2e57…'` + `source_registry_row='wb.flk.npc.gov.cn / SCANNED_PDF_RESEARCH / S0'` + `source_registry_sha256` + `demo_reason=null` + `source_file_url='(OCR_SCAN_FROM_S0_REGISTRY:executor_587:2026-08-29Tnow+08:00)'` + `real_pdf_path='/tmp/cegr_uploads/shaanxi_fiscal_regulation_flk.pdf'` + `purpose_note='中文 OCR 压力研究样本（陕西财政预算管理条例）；O3 §5.2.6 真实 OCR 收口用例'`）+ 执行端自验（SHA 验证 + validate_ocr_input ACCEPT + paddle-ocr MOCK 调用链 + source_document row dict + lineage JSONB schema 合规）；**零用户动作 / 零用户裁定 / 零用户亲验 / 零 `--confirm-o3=PATH` 字面**（per 2026-08-29 治理铁律；supersede 旧版 `587-stage0-architect-s586-o3-impl-real-pdf-user-action-tasking-20260829.md`「用户提供真实 PDF」假设作废；per `587-stage0-cc-o3-impl-real-pdf-e2e-tasking-20260829-receipt`；588 架构师审计 PASS 后宣布 O3 整体 CLOSED 候选）|
...
> [superseded per 593（2026-08-29）· per 2026-08-29 治理铁律：用户无 PDF 数据；数据源唯一=政府/统计局/研究机构自取；执行端自取预 vetted 源走完整 e2e 流水线；零 `--confirm-*` 字面；user-action 表述保留为治理教训（不删除、不调用），仅当执行端自取路径无法取得样本时方由架构师夜间授权下自主评估是否启动；O1 §5.2.x 真实 SHA-locked 江苏样本刀待 docs/52 B 路落定后另刀下发；O3 §5.2.6 真实 PDF e2e 收口闭合 per `587-stage0-architect-s586-o3-impl-real-pdf-self-sourced-tasking-20260829.md`（supersede 旧版 `587-stage0-architect-s586-o3-impl-real-pdf-user-action-tasking-20260829.md`）+ `587-stage0-cc-o3-impl-real-pdf-e2e-tasking-20260829-receipt.md`（执行端自取 S0 源 + paddle-ocr MOCK only + 执行端自验闭环）+ `588-stage0-architect-s587-o3-impl-real-pdf-e2e-audit-PASS-20260829.md` PASS audit + §J ⚠1 ACCEPTED with disclosure 标注 + `589-stage0-architect-s588-o3-impl-docs50-supersede-refresh-tasking-20260829.md` O3 row 119 supersede 平行模式 + `590-stage0-architect-s589-o3-impl-docs50-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 line 121 vs 120 ACCEPTED with disclosure 标注 + `591-stage0-architect-s590-o3-impl-docs50-o1-row117-supersede-refresh-tasking-20260829.md` O1 row 117 A 路 supersede 平行模式 + `592-stage0-architect-s591-o1-impl-docs50-o1-row117-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 ACCEPTED with disclosure + ⚠2 ACCEPTED with disclosure 标注；**O3 整体 CLOSED 候选** per 588 PASS + 590 PASS 双重声明；**O1 整体仍 WAITING_FILE**（per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；O1 §5.2.x 真实 SHA-locked 江苏样本刀待 docs/52 B 路落定后另刀下发；B 路（公开源自动获取）保持主路径）；584 重 ACK 触发条件保留 = 用户裁定 + Python 3.12 wheel 可用 + Docker daemon 就绪 + 项目主 deps manifest 决策已定 + Dockerfile（非 current critical path）；本 docs/49 §5.3 row O1 真实 SHA 原文不删不改（WAITING_FILE + 等用户 `--confirm-o1=PATH` 表述保留为治理教训），supersede 标注与原文共存（per 「不删既有 OPEN 行」红线 + 「不删旧 row」教训模式 + 589 + 591 平行模式）]
...
> [superseded per 593（2026-08-29）· per 2026-08-29 治理铁律：用户无 PDF 数据；数据源唯一=政府/统计局/研究机构自取；执行端自取预 vetted 源走完整 e2e 流水线；零 `--confirm-*` 字面；user-action 表述保留为治理教训（不删除、不调用），仅当执行端自取路径无法取得样本时方由架构师夜间授权下自主评估是否启动；O1 §5.2.x 真实 SHA-locked 江苏样本刀待 docs/52 B 路落定后另刀下发；O3 §5.2.6 真实 PDF e2e 收口闭合 per `587-stage0-architect-s586-o3-impl-real-pdf-self-sourced-tasking-20260829.md`（supersede 旧版 `587-stage0-architect-s586-o3-impl-real-pdf-user-action-tasking-20260829.md`）+ `587-stage0-cc-o3-impl-real-pdf-e2e-tasking-20260829-receipt.md`（执行端自取 S0 源 + paddle-ocr MOCK only + 执行端自验闭环）+ `588-stage0-architect-s587-o3-impl-real-pdf-e2e-audit-PASS-20260829.md` PASS audit + §J ⚠1 ACCEPTED with disclosure 标注 + `589-stage0-architect-s588-o3-impl-docs50-supersede-refresh-tasking-20260829.md` O3 row 119 supersede 平行模式 + `590-stage0-architect-s589-o3-impl-docs50-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 line 121 vs 120 ACCEPTED with disclosure 标注 + `591-stage0-architect-s590-o3-impl-docs50-o1-row117-supersede-refresh-tasking-20260829.md` O1 row 117 A 路 supersede 平行模式 + `592-stage0-architect-s591-o1-impl-docs50-o1-row117-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 ACCEPTED with disclosure + ⚠2 ACCEPTED with disclosure 标注；**O3 整体 CLOSED 候选** per 588 PASS + 590 PASS 双重声明；**O1 整体仍 WAITING_FILE**（per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；O1 §5.2.x 真实 SHA-locked 江苏样本刀待 docs/52 B 路落定后另刀下发；B 路（公开源自动获取）保持主路径）；584 重 ACK 触发条件保留 = 用户裁定 + Python 3.12 wheel 可用 + Docker daemon 就绪 + 项目主 deps manifest 决策已定 + Dockerfile（非 current critical path）；本 docs/49 §6.3 row O1 真实 SHA 阻塞项 原文不删不改（`用户主动 --confirm-o1=PATH` 表述保留为治理教训），supersede 标注与原文共存（per 「不删既有 OPEN 行」红线 + 「不删旧 row」教训模式 + 589 + 591 平行模式）]
| ❌ O3 真实 PDF 未提供 | O3 流水线无端到端验证 | 用户主动 `--confirm-o3=PATH` |

> [superseded per 593（2026-08-29）· per 2026-08-29 治理铁律：用户无 PDF 数据；数据源唯一=政府/统计局/研究机构自取；执行端自取预 vetted 源走完整 e2e 流水线；零 `--confirm-*` 字面；user-action 表述保留为治理教训（不删除、不调用），仅当执行端自取路径无法取得样本时方由架构师夜间授权下自主评估是否启动；O1 §5.2.x 真实 SHA-locked 江苏样本刀待 docs/52 B 路落定后另刀下发；O3 §5.2.6 真实 PDF e2e 收口闭合 per `587-stage0-architect-s586-o3-impl-real-pdf-self-sourced-tasking-20260829.md`（supersede 旧版 `587-stage0-architect-s586-o3-impl-real-pdf-user-action-tasking-20260829.md`）+ `587-stage0-cc-o3-impl-real-pdf-e2e-tasking-20260829-receipt.md`（执行端自取 S0 源 + paddle-ocr MOCK only + 执行端自验闭环）+ `588-stage0-architect-s587-o3-impl-real-pdf-e2e-audit-PASS-20260829.md` PASS audit + §J ⚠1 ACCEPTED with disclosure 标注 + `589-stage0-architect-s588-o3-impl-docs50-supersede-refresh-tasking-20260829.md` O3 row 119 supersede 平行模式 + `590-stage0-architect-s589-o3-impl-docs50-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 line 121 vs 120 ACCEPTED with disclosure 标注 + `591-stage0-architect-s590-o3-impl-docs50-o1-row117-supersede-refresh-tasking-20260829.md` O1 row 117 A 路 supersede 平行模式 + `592-stage0-architect-s591-o1-impl-docs50-o1-row117-supersede-refresh-audit-PASS-20260829.md` PASS audit + ⚠1 ACCEPTED with disclosure + ⚠2 ACCEPTED with disclosure 标注；**O3 整体 CLOSED 候选** per 588 PASS + 590 PASS 双重声明（per §5.2.6 CLOSED per 587 + §5.2.5 CLOSED per 585 + §5.2.4 BLOCKED-DEFERRED per 584）；**O1 整体仍 WAITING_FILE**（per docs/47 §3.1 + 用户 2026-08-26 披露 + 2026-08-29 治理铁律；B 路（公开源自动获取）保持主路径）；584 重 ACK 触发条件保留 = 用户裁定 + Python 3.12 wheel 可用 + Docker daemon 就绪 + 项目主 deps manifest 决策已定 + Dockerfile（非 current critical path）；本 docs/49 §6.3 row O3 真实 PDF 阻塞项 原文不删不改（`用户主动 --confirm-o3=PATH` 表述保留为治理教训），supersede 标注与原文共存（per 「不删既有 OPEN 行」红线 + 「不删旧 row」教训模式 + 589 + 591 平行模式）]
