...
- **架构师裁定：路径 C（维持现状 + 转向 M4-M5）**。详见 `docs/57-m3-launch-conditions-review-20260901.md`。
- **理由速记**：M3 数据依赖项（NBS API / tjj.*.gov.cn）636 probe 实测 0 REACHABLE，本机 IP 被 .gov.cn WAF IP-level 阻断；路径 A 需用户登录/提供源（违反数据源治理铁律「执行端不可提任何用户裁定事项」）；路径 B 商业年鉴库（U4 暂禁）。路径 C 不依赖用户提供源 / 不接商业库；可立即启动 M4（人物政策）+ M5（分析方法）。
- **M3 重启条件**（任一）：用户本地浏览器导出 PDF/HTML / 用户重审 U4 / WAF 解封。
- **下一步**：638 = M4.1 人物表 schema 收口 + 政府工作报告数据可得性 probe（架构师 tasking 在 637 接受后签发）。
- **不宣布 Gate / O1 / M2 PASS**。
