...
> **637 架构师裁定（2026-09-01）**：`docs/57-m3-launch-conditions-review-20260901.md`。M3 默认数据源（tjj.*.gov.cn 公报）在 636 probe 实测 0/744 REACHABLE（本机 IP `125.93.9.191` 被 .gov.cn WAF IP-level 阻断；非短期可解）。**架构师推荐：路径 C（维持 M2 现状 + 转向 M4-M5）**。M3 数据依赖项硬阻断；M4 / M5 无数据依赖项可独立推进。M3 重启条件（任一）：用户本地浏览器导出 PDF/HTML / 用户重审 U4（商业库授权）/ WAF 解封。
...
| U6 | 红黑统计公报库（tjgb.hongheiku.com 转载库）可否作为 M2/M3 observation 数据源？（用户 2026-09-02 主动提供 URL + 裁定「接受（含金丝雀守门）」） | **接受**。附守门：5 省金丝雀核对（京/沪/鲁/鄂/川 5/5 一致才批量）+ 国家锚核对 + lineage 全行标注转载出处与 U6 + SHA 锁转载字节。详见 `docs/81-u6-hongheiku-source-ruling-20260902.md`。解锁 M2.1 补 26 省 / M2.2 三次产业 / M2.3 升级评估 / M2.4（2021-2025）/ M3 数据依赖放宽。不构成任何 Gate/O1/M2 PASS。 |
