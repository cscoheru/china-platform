...
> 🔗 **第 30 项（此条）· SHA drift 处置 (a) 裁定执行登记（registry NATIONAL_BULLETIN 行已更；本条为文档登记节点）**（per `540` cc 回执；queue_rev 288 落地）：登记 per `538` 已执行事实——registry NATIONAL_BULLETIN 行 `file_hash_sha256` → `a7e4029d…` + `file_size_bytes` → 180165（用户 2026-08-27 裁定 (a)——认定源站换版，per 回执 `510` live-probe 实测）；live 复验由用户/Cursor 本机完成 exit 0、download 180165 B、sha256 与 expected 匹配（per `538-stage2-cursor-local-live-reverify-and-deviation-accept-20260827` §1 + D1–D5 偏差交付接受）；本条只登记 (a) 执行事实：registry 本刀零改动、第 21–29 项既有正文原样未动、不改代码、不动 4 fixture 字节（`nbs=e30ee811` / `nbs_live=9232efdb` / `sz=937255a5` / `hb=9056001c`，与前序各刀锁值完全一致）；**O1 仍 OPEN——registry 更新 ≠ O1 收口（mart 真 SHA 未入仓）；非 O1/Gate PASS；不换服务器**。
