...
| ❌ 索要 PAT | ✅ |
| ✅ pack invariant | ⏳ bump + commit 后 720 == 720 == 720（本刀 docs/45 + docs/53 登记 + 回执 407 + bump → 718 → 720；+2 = bump + receipt；docs/45/docs/53 皆 SHA REFRESH 不增计数；knife 70 = 四轨 CSV 静态下载（scripts/gen_public_extracts_csv.py + 4 CSV 产物 + smoke §12i 门 15 针 + 13 pytest + 回执 404）710 → 718；knife 69 = docs/45+53 行筛选登记 708 → 710；knife 68 = 四轨客户端行筛选（page.tsx `"use client"` + 4 useState + filterRows + TrackFilterInput + smoke §12h 门 11 针 + 3 pytest + 回执 398）706 → 708；knife 67 = 湖北首页轨链 703 → 706；knife 66 = 深圳城页链 700 → 703；knife 65 = JSON 静态下载 694 → 700；knife 64 = docs/45+53 overview 登记 692 → 694；knife 63 = overview strip 690 → 692）|
