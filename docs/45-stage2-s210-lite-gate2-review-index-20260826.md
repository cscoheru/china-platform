...
**公开源自动获取 + ops 手册 + 公开提取三轨**（per `docs/52-stage2-official-open-source-auto-ingest-plan-20260826.md`（规划；queue_rev 137）+ `docs/53-stage2-public-ingest-ops-handbook-20260826.md`（ops 手册；回执 `365`；queue_rev 155 补三轨预览注记））：stats.gov.cn 试点 connector（`scripts/auto_ingest_public_source.py`）四种模式（dry-run / `--from-local-sample` / `--live --confirm-live` / `--refresh-live-candidate`）+ 10 出口码契约；NBS **双轨** = REGISTRY_SAMPLE sample 轨（63 行 / `dea13b8a…`，registry 锚定，`intake_status=REGISTRY_SAMPLE_INTAKED`，is_demo=true）↔ LIVE_CANDIDATE 候选轨（60 行 / `0b85212f…`，WORM `zxfb` 归档锚定，`intake_status=LIVE_CANDIDATE`，is_demo=true）；**深圳 REGISTRY_SAMPLE 第三轨**（回执 `368`→`371`）：sz.gov.cn MUNICIPAL_BULLETIN 本地样本散文段落提取（公报正文纯散文、数据表以 PNG 图片嵌入 → 71 行 `{section, paragraph}` / `d5e2c731…` registry 锚 / WORM `data/public_archives/2026-08/sz.gov.cn/sample.html`）+ fixture `frontend/lib/public_extract_sz.json`（byte-verbatim 快照）+ `/public-extracts` 第三分节（DemoBadge + SSL 暂缓非 live 免责）；三轨互不覆盖（pytest 字节级锁定 + smoke §12c/§12d 交叉检查）；预览 `/public-extracts`（静态分节，显式标注 demo/candidate）。回执链：`350`（前端 sample 区块）→ `353`（pytest 提取保护）→ `356`（JS 壳启发式 + NBS live 过壳）→ `359`（live WORM 提取 + LIVE_CANDIDATE 前端区块）→ `362`（一键刷新 CLI）→ `368`（深圳散文抽取修复 0→71 行）→ `371`（深圳前端分节）。**三轨皆 demo/candidate 演示：live SHA drift 等 user 裁定，不自动改 registry、不自动 O1 收口**（per knife 333 drift 契约 + `364`/`373` §红线）。
...
| ❌ 索要 PAT | ✅ |
| ✅ pack invariant | ⏳ bump + commit 后 684 == 684 == 684（knife 60: docs/45 三轨刷新 + docs/53 预览注记 + 回执 374 + bump；682 → 684；+2 = bump + receipt；docs/45/53 SHA REFRESH 不增计数；前置 knife 59 = 深圳前端分节 679 → 682；knife 58 = 深圳散文抽取修复 676 → 679；knife 57 = docs/53 登记 673 → 676；knife 55/56 = 候选双轨 + 刷新 CLI 667 → 673；knife 41-44 = docs/50/51 系列 635 → 643）|
