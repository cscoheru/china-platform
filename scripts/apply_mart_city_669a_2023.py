#!/usr/bin/env python3
"""
669a-2023 mart apply — real data harvest + 多刀 lineage ruling 守门
========================================================================

Per 663 Gap 1: dbt CLI 工具链与 Python 3.14 不兼容,
直接用 psycopg2 跑 SQL 应用 mart_city_timeseries (knife 669a-2023 real-data sub-knife).

State after 669a-2020 + 669a-2021 + 669a-2022 + 669a-2023:
  - 280 rows = 4 city × 10 indicator × 7 year (2020-2026)
  - real_cells = 100 (26 from 2021 + 37 from 2022 + 37 from 2023)
  - DATA_MISSING = 180 (40 for 2020 + 14 for 2021 missing + 3 for 2022 missing
                        + 3 for 2023 missing + 120 for 2024-2026)
  - ruling_versions = 5 (K669a-2020 / 2021 / 2022 / 2023 + pending)
"""
...
"""
    ...
    return {
        ...
        "by_year": year_rows,
        "city_2023_real": city_2023_real,
    }


def main():
    print(f"=== knife 669a-2023 mart apply (real-data harvest) ===")
    ...
    try:
        ...
        print(f"  real_cells       = {s['real_cells']}    (expect 100 = 26[2021] + 37[2022] + 37[2023])")
        print(f"  DATA_MISSING     = {s['data_missing_cells']}  (expect 180 = 40[2020] + 14[2021 miss] + 3[2022 miss] + 3[2023 miss] + 120[2024-2026])")
        print(f"  ruling_versions  = {s['ruling_versions']}      (expect 5 = K669a-2020/2021/2022/2023 + pending)")
        print()
        print("=== by year row count ===")
        for yr, cnt in s['by_year']:
            print(f"  year {yr}: {cnt} rows")
        print()
        print("=== 2023 real cells by city ===")
        for cc, cnt in s['city_2023_real']:
