"""M4.12 政策详情 v6 真实化 spike fetch (knife 649 §A.1, 2026-09-01).

Per knife 649 tasking §1.649-A.1:
- 2 真实样本: hubei + jilin (第 11/12 样本)
...
- chain_id = 'real_649_m4_12_policy_detail_v6' (末段 `_v6` ≠ 648 `_v5` ≠ 647 `_v4`)
- UUID h 段 (h0eebc99-h6eebc99) ≠ 648 g 段 ≠ 647 f 段
- 已用省全集 (不得重复): HLJ / HENAN / YUNNAN / FUJIAN / GD / ZJ / JX / HUN / AH
- 2 NEW SHA 全 distinct ≠ 638-648 全部 SHA
- lineage 全 is_demo='false' 真实化 sentinel (per docs/33 §3.2)
- 16 INSERT total = 12 政策表 + 2 source_registry + 2 source_document
- ≤12 HTTP total (per 649 §A.1 总预算)
...
# 首选 + fallback 1 (省府根) + 递补池 (按序)
HUBEI_FALLBACK_CHAIN: list[tuple[str, str]] = [
    ("https://www.hubei.gov.cn/zwgk/", "zwgk_root"),
    ("https://www.hubei.gov.cn/", "province_root"),
]
JILIN_FALLBACK_CHAIN: list[tuple[str, str]] = [
    ("https://www.jl.gov.cn/zwgk/", "zwgk_root"),
    ("https://www.jl.gov.cn/", "province_root"),
]
SUBSTITUTE_POOL: list[tuple[str, list[tuple[str, str]], str]] = [
    ("liaoning", [
        ("https://www.ln.gov.cn/zwgk/", "zwgk_root"),
        ("https://www.ln.gov.cn/", "province_root"),
    ], "辽ICP备"),
    ("shaanxi", [
    ...
    ("jiangsu", [
        ("https://www.js.gov.cn/zwgk/", "zwgk_root"),
        ("https://www.js.gov.cn/", "province_root"),
    ], "苏ICP备"),
]
...
# 锚点正则
ANCHOR_RE = (
    r"湖北|jiangxi|jiangsu|政务公开|政府公报|政府文件|政策法规|公开目录|领导信息"
)
    ...
    try:
        proc = subprocess.run(
            ["curl", "-sS", "-L", "--max-time", str(timeout),
             "-o", "/tmp/_649_fetch_body", "-w", "%{http_code}|%{errormsg}",
             url],
        ...
        try:
            ...
            code = 0
        body = Path("/tmp/_649_fetch_body").read_bytes()
...
def fetch_cell(
    ...
    Returns cell with:
    - province: original requested province (e.g., "hubei") — even after substitute
    - actual_province: where data was actually fetched from (e.g., "liaoning" if substitute)
            ...
            if code == 200 and size > 1000 and anchor_hits >= 1 and not waf_present:
                return {
                    ...
                    "substitute_reason": (
                        ...
                        f"按 649 任务书 §0.13 递补池按序取 {sub_province}"
                    ),
                }
    # 全部失败
    return {
...
def main() -> int:
    ...
    # Cell 2: jilin
    cell_jilin = fetch_cell("jilin", JILIN_FALLBACK_CHAIN, http_used)

    cells = [cell_hubei, cell_jilin]
    ...
    output = {
        "knife": "649-A.1",
        "purpose": "M4.12 政策详情 v6 真实化 spike (hubei + jilin 第 11/12 样本, 预授权池激活)",
        "chain_id": "real_649_m4_12_policy_detail_v6",
        "uuid_prefix": "h",
        "uuid_prefixes": {
            "source_registry": "h0eebc99",
            "source_document": "h0eebc99",
            "policy_document": "h1eebc99",
            "policy_target": "h2eebc99",
            "policy_measure": "h3eebc99",
            "government_commitment": "h4eebc99",
            "commitment_progress": "h5eebc99",
            "project_event": "h6eebc99",
        },
        "summary": {
        ...
        "fetch_log": fetch_log,
        "methodology": (
            "v6 spike fetch: 2 cells (hubei + jilin), each with primary /zwgk/ + "
            "fallback #1 (省府根 /) + 递补池 (liaoning→shaanxi→sichuan→guizhou→jiangsu). "
            "每 cell ≤4 attempts, 总预算 ≤12 HTTP. "
            "Per 649 §0.13: 附属复验/验证产物允许独立文件, 但主 evidence methodology 必须含指针."
        ),
    }

    out_path = Path("evidence_pack/m4_12_policy_detail_real_v6_20260901.json")
