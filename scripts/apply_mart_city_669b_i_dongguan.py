#!/usr/bin/env python3
"""669b-i-dongguan mart apply — UPDATE-ONLY psycopg2 pattern.
...
(47 real HONGHEIKU_TRANSLOAD + 13 DATA_MISSING), UPDATEs all 60 cells with correct
...
SEED_CSV = "/tmp/669b_i_cache/seed_dongguan_full.csv"
CITY_CODE = "GUANGDONG_DONGGUAN"
DONGguan_EID = {2021: 25333, 2022: 42065, 2023: 47430, 2025: 69935}
...
def main():
    print(f"=== knife 669b-i-dongguan mart apply (UPDATE-ONLY) ===")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for indicator, year, value in real_rows:
                ...
                eid = DONGguan_EID.get(year, "")
                lineage_origin = f"tjgb.hongheiku.com/djs/{eid}.html" if eid else ""
                lineage_ruling = f"K669b-i-dongguan-parse-{year}-2026-09-10"
                cur.execute(f"""
            ...
            for indicator, year, missing_reason, lineage_ruling in miss_rows:
                if year == 2020:
                    ...
                    lineage_origin = "tjgb.hongheiku.com/tag/东莞市 (no 2020 entry, 守新增红线-3 不手填)"
                    new_ruling = "K669b-i-dongguan-no-bulletin-2020-2026-09-10"
                else:
                    # fixed_asset cells (2021/2022/2023): bulletin 仅发增长%
                    lineage_origin = f"tjgb.hongheiku.com/djs/{DONGguan_EID.get(year, '')}.html (bulletin 仅发增长%)"
                    new_ruling = "K669b-i-dongguan-parse-fixed_asset_growth_pct-2026-09-10"
                cur.execute(f"""
                    UPDATE {TARGET_SCHEMA}.{MART_NAME}
                    SET value = NULL,
                        status = 'DATA_MISSING',
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"SELECT COUNT(*) FROM {TARGET_SCHEMA}.{MART_NAME} WHERE city_code = %s AND lineage_ruling LIKE 'K669b-i-dongguan-%%' AND value IS NOT NULL", (CITY_CODE,))
            ruling_real = cur.fetchone()[0]
            cur.execute(f"SELECT COUNT(*) FROM {TARGET_SCHEMA}.{MART_NAME} WHERE city_code = %s AND lineage_ruling LIKE 'K669b-i-dongguan-%%' AND value IS NULL", (CITY_CODE,))
        ...
        print(f"\n=== Apply summary ===")
        print(f"  DONGGUAN total:       {real_total + miss_total} cells ({real_total} real + {miss_total} missing)")
        print(f"  K669b-i-dongguan:     {ruling_real} real + {ruling_miss} missing")
        print(f"\n  Per-year breakdown:")
