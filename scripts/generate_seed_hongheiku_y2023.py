#!/usr/bin/env python3
"""665c generate_seed_hongheiku_timeseries_2023.py — build seed CSV from parse JSON.

Mirror 665b seed CSV schema (dbt seed format):
...
Sources:
  - evidence_pack/u6_batch_y2023_parse_20260904.json (extract 10 indicators per province)
  - evidence_pack/u6_batch_y2023_fetch_20260904.json (URL per province)

Output:
  dbt/seeds/seed_hongheiku_timeseries_2023.csv (~270 rows; 30/31 provinces PARSED;
  GUANGDONG 2023 PDF目录页 PARSE_EMPTY → no row written → DATA_MISSING 守红线-1)
"""
import csv
import json
...
# Province CN names (mirror 665a/665b seed)
PROVINCE_NAME_CN = {
...
def main() -> int:
    ...
    # Build URL lookup from fetch results
    ...
    for cell in parse_data['cells']:
        ...
        for ind_key, value in cell['extracted'].items():
            label_cn, unit = INDICATOR_LABELS[ind_key]
            rows.append({
