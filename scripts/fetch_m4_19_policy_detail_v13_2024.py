"""M4.19 政策详情 v13 华南双省对 spike fetch (knife 656 §A.1, 2026-09-02).

Per knife 656 tasking §1.656-A.1:
- 2 真实样本: GUANGXI + HAINAN (第 25/26 样本; 华南双省对; 无前史首试 → retry_of=N/A)
...
- 两省无前史首试 (vs 651/652/655 双首试) → retry_of 不适用 (首试省); 若 BLOCKED → 纯 BLOCKED_NO_POOL 留痕
...
- 已用省全集 (不得重复, 按 actual_province 口径, 19 省 after 655):
  ...
  656 增量后 = 20/21 省 (双 REACHABLE → 21 省)
  注: 留 HEBEI/SHANXI 给 657 全国 31 省收官
- 2 NEW SHA 全 distinct ≠ 638-655 全部 SHA (or BLOCKED cell 留 blocked_reason 无 SHA)
- lineage 全 is_demo='false' 真实化 sentinel (per docs/33 §3.2)
- ≤12 HTTP total (per 656 §0.3 红线 3 + 全刀预期 2-4)
...
# 首选 + fallback 1 (省府根)
GUANGXI_FALLBACK_CHAIN: list[tuple[str, str]] = [
    ("https://www.gxzf.gov.cn/zwgk/", "zwgk_root"),
    ("https://www.gxzf.gov.cn/", "province_root"),
]
HAINAN_FALLBACK_CHAIN: list[tuple[str, str]] = [
    ("https://www.hainan.gov.cn/zwgk/", "zwgk_root"),
    ("https://www.hainan.gov.cn/", "province_root"),
]
# 递补池 (per 656 §0.14 红线 14 沿用 655 耗尽态; 656 首试省若两级 fallback 全失败 → BLOCKED_NO_POOL 留痕)
SUBSTITUTE_POOL: list[tuple[str, list[tuple[str, str]], str]] = []
SUBSTITUTE_POOL_STATUS = "EXHAUSTED"  # per 红线 14 增补; 656 沿用 655 耗尽态

# 首试省 lineage 注解 (per 656 §0.14 红线 14 + 656 tasking §1.656-A.1)
# 无前史 → retry_of 不适用 (留空); 若 BLOCKED → 纯 BLOCKED_NO_POOL 留痕
RETRY_OF_NOTES: dict[str, str] = {
    "guangxi": "retry_of=N/A (无前史首试; per 656 §0.14)",
    "hainan": "retry_of=N/A (无前史首试; per 656 §0.14)",
}
...
def _curl(url: str, timeout: int = TIMEOUT) -> tuple[int, str, str]:
    ...
    """
    body_path = Path(f"/tmp/_656_fetch_{abs(hash(url)) % 100000}")
    try:
        proc = subprocess.run(
        ...
        try:
            ...
            code = 0
        # Read body if exists, else use curl errormsg
        if body_path.exists():
...
def fetch_cell(
    ...
    Per 红线 14 沿用 655: 递补池耗尽; 两级 fallback 全失败 → BLOCKED 留痕, 不跨省代换。
    656 §0.14 首试省: 该分支代码必须存在并可达, 无论 REACHABLE 还是 BLOCKED;
    三态均合法 (双 REACHABLE → 16 INSERT + 2 NEW SHA; 混合 → 按省实报; 双 BLOCKED → 0 INSERT + 三重留痕)。
    GUANGXI/HAINAN 无前史 → retry_of 不适用 (首试省; lineage 留空留痕)。
    """
    ...
    # 真网首试省 BLOCKED_NO_POOL 留痕 (per 656 §0.14 红线 14 沿用 655; 656 双省无前史 → retry_of 不适用)
...
def main() -> int:
    ...
    # Cell 1: guangxi (首试; 无前史)
    cell_guangxi = fetch_cell("guangxi", GUANGXI_FALLBACK_CHAIN, http_used)
    # Cell 2: hainan (首试; 无前史)
    cell_hainan = fetch_cell("hainan", HAINAN_FALLBACK_CHAIN, http_used)

    cells = [cell_guangxi, cell_hainan]
    ...
    output = {
        "knife": "656-A.1",
        "purpose": (
            "M4.19 政策详情 v13 华南双省对 spike (guangxi + hainan 第 25/26 样本; "
    ...
    out_path = Path("evidence_pack/m4_19_policy_detail_real_v13_20260902.json")
