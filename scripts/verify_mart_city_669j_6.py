...
knife 669j sub-knife 6/6 (2026-09-13): 6 city cross-province NO-OP
  SICHUAN_MIANYANG, SICHUAN_DEYANG, SICHUAN_NANCHONG,
  SHAANXI_BAOJI, SHAANXI_XIANYANG, LIAONING_ANSHAN

baseline (post-669j-5): 62 cities, 4340 rows, 1146 real, 3194 miss, 60 ruling_versions
post-669j-6 expectation (delta-based):
  - cities 68 (62 + 6)
  - rows 4760 (68 × 10 × 7)
  - 6 city cells 420
  - 6 city real 0 (NO-OP)
  - 6 city MISSING 420
  - 6 lineage_ruling 669j-* versions
  - per-province: SICHUAN +3 city, SHAANXI +2 city, LIAONING +1 city
"""
import os
import sys
import psycopg2
...
def main():
    ...
    city_list = ",".join(f"'{c}'" for c in CITY_6)

    n = q(cur, f"SELECT COUNT(*) FROM {SCHEMA}.{TABLE}")[0]
    if n == 4760:
        p(f"row count = 4760 (68 × 10 × 7)")
    else:
        f(f"row count = {n}, expected 4760")

    n = q(cur, f"SELECT COUNT(DISTINCT city_code) FROM {SCHEMA}.{TABLE}")[0]
    if n == 68:
        p(f"city distinct = 68 (62 prior + 6)")
    else:
        f(f"city distinct = {n}, expected 68")
    ...
    if n == 6:
        p(f"669j-* 唯一 lineage_ruling = 6 (K669j-1/2/3/4/5/6)")
    else:
        f(f"669j-* rulings = {n}, expected 6")
    ...
    if n == 6:
        p(f"6 city distinct = 6")
    else:
        f(f"6 city distinct = {n}")

    n = q(cur, f"SELECT COUNT(*) FROM {SCHEMA}.{TABLE} WHERE city_code IN ({city_list})")[0]
    if n == 420:
        p(f"6 city cells = 420")
    else:
        f(f"6 city cells = {n}")

    for tag in ["669j-6", "hongheiku 0 entry", "6 HTTP", "不手填"]:
        n = q(cur, f"""
            SELECT COUNT(*) FROM {SCHEMA}.{TABLE}
        ...
        """)[0]
        if n == 420:
            p(f"missing_reason 含 '{tag}' (420)")
        else:
            f(f"missing_reason '{tag}' = {n}/420")

    bad = q(cur, f"""
        SELECT COUNT(*) FROM {SCHEMA}.{TABLE}
        WHERE city_code IN ({city_list})
          AND lineage_ruling != 'K669j-6-2026-09-13'
    """)[0]
    if bad == 0:
        p(f"6 city 全 lineage_ruling = 'K669j-6-2026-09-13'")
    else:
    ...
    """)[0]
    if bad == 0:
        p(f"6 city 全 lineage_source_type = 'DATA_MISSING'")
    else:
        f(f"lineage_source_type 异常: {bad}")

    n = q(cur, f"SELECT COUNT(DISTINCT lineage_ruling) FROM {SCHEMA}.{TABLE} WHERE lineage_ruling LIKE '%669j-6%'")[0]
    if n == 1:
        p(f"K669j-6-* 唯一版本")
    else:
        f(f"K669j-6 rulings 不唯一 = {n}")
    ...
    if n == 420:
        ...
        f(f"lineage_origin 'tjgb.hongheiku.com' = {n}/420")
    ...
    n = q(cur, f"""
        SELECT COUNT(*) FROM {SCHEMA}.{TABLE}
        WHERE city_code IN ({city_list})
          AND (lineage_origin LIKE '%MIANYANG%' OR lineage_origin LIKE '%DEYANG%'
               OR lineage_origin LIKE '%NANCHONG%' OR lineage_origin LIKE '%BAOJI%'
               OR lineage_origin LIKE '%XIANYANG%' OR lineage_origin LIKE '%ANSHAN%')
    """)[0]
    if n == 420:
        p(f"lineage_origin 含 6 city 名")
    else:
        f(f"lineage_origin 含 city 名 = {n}/420")
    ...
    for yr in [2020, 2021, 2022, 2023, 2024, 2025, 2026]:
        ...
        """)[0]
        if n_miss == 60 and n_real == 0:
            p(f"6 city × {yr} = 60 MISS, 0 real")
        else:
            f(f"6 city × {yr} miss={n_miss} real={n_real}")
    ...
    """)[0]
    if n_real == 0 and n_miss == 420:
        p(f"NO-OP 不手填: real=0, MISS=420")
    else:
    ...
    if n_2020 == 0 and n_2026 == 0:
        p(f"6 city 2020 + 2026 全 MISSING")
    else:
        f(f"6 city 2020/2026 real: {n_2020}/{n_2026}")
    ...
    """)[0]
    if n_prov == 3:
        p(f"6 city 跨 3 province (SICHUAN + SHAANXI + LIAONING)")
    else:
        f(f"6 city 跨 {n_prov} province, expected 3")

    n = q(cur, f"""
        SELECT COUNT(DISTINCT city_code) FROM {SCHEMA}.{TABLE}
        WHERE province_code = 'SICHUAN' AND city_code IN ({city_list})
    """)[0]
    if n == 3:
        p(f"SICHUAN 新增 3 city (MIANYANG/DEYANG/NANCHONG)")
    else:
        f(f"SICHUAN 新增 city = {n}")

    n = q(cur, f"""
        SELECT COUNT(DISTINCT city_code) FROM {SCHEMA}.{TABLE}
        WHERE province_code = 'SHAANXI' AND city_code IN ({city_list})
    """)[0]
    if n == 2:
        p(f"SHAANXI 新增 2 city (BAOJI/XIANYANG)")
    else:
        f(f"SHAANXI 新增 city = {n}")

    n = q(cur, f"""
        SELECT COUNT(DISTINCT city_code) FROM {SCHEMA}.{TABLE}
        WHERE province_code = 'LIAONING' AND city_code IN ({city_list})
    """)[0]
    if n == 1:
        p(f"LIAONING 新增 1 city (ANSHAN)")
    else:
        f(f"LIAONING 新增 city = {n}")

    n = q(cur, f"""
        SELECT COUNT(*) FROM {SCHEMA}.{TABLE}
        WHERE lineage_ruling LIKE '%669j%'
    """)[0]
    if n == 2170:
        p(f"669j program 累计 cells = 2170 (31 city × 7 year × 10 indicator)")
    else:
        f(f"669j program cells = {n}, expected 2170")
    ...
    """)[0]
    if n == 31:
        p(f"669j program 累计 distinct city = 31 (5+5+5+5+5+6)")
    else:
        f(f"669j program city = {n}, expected 31")

    cur.close()
    conn.close()

    print()
    print(f"=== knife 669j-6 红线 summary: {PASS}/{PASS+FAIL} PASS, {FAIL} FAIL ===")
