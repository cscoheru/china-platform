"""M4.5 — 6 REACHABLE 任免源真实化 fetch (knife 642 Block A.2, M4.5 side).

Per knife 642 §2.642-A.2:
- 6 REACHABLE 任免源 (639 验证) × 1 detail each = 6 detail fetches
- 真实 SHA256 计算
- 写入 evidence_pack/m4_5_renmian_real_20260901.json
...
6 试点省 REACHABLE 任免源 (639 evidence):
- hlj /zwgk/ (实际政务公开 landing /hlj/c108368/zwgk.shtml per 641)
- fujian /zwgk/
- henan /zwgk/
- guangdong /zwgk/ (gd.gov.cn)
- guizhou /zwgk/
- yunnan /zwgk/ (yn.gov.cn)
...
# 6 试点省 (639 验证 REACHABLE)
PROVINCES = [
    ("heilongjiang", "https://www.hlj.gov.cn/hlj/c108368/zwgk.shtml"),       # 641 验证政务公开 landing
    ("fujian",       "https://www.fujian.gov.cn/zwgk/"),
    ("henan",        "https://www.henan.gov.cn/zwgk/"),
    ("guangdong",    "https://www.gd.gov.cn/zwgk/"),
    ("guizhou",      "https://www.guizhou.gov.cn/zwgk/"),
    ("yunnan",       "https://www.yn.gov.cn/zwgk/"),
]
TIMEOUT = 15
HTTP_LIMIT = 12
...
# 任免通知关键词
RENMIAN_RE = re.compile(
    r"任免职|任免通知|任命|免职|任免|appoint|removal", re.IGNORECASE,
)


def extract_detail_link(html: bytes, base_domain: str) -> dict | None:
    """Extract first 任免 detail URL from landing page."""
    try:
    ...
    for m in LINK_RE.finditer(text):
        ...
        if base_domain not in href:
            continue
        if not RENMIAN_RE.search(anchor):
            continue
...
def run_fetch() -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "fetch_methodology": (
                f"M4.5 任免真实化; 6 试点省 × 1 detail each = {len(cells)} real samples; "
...
def write_outputs(results: dict) -> None:
    ...
    lines = [
        ...
        "",
        f"总抓取: {sv.get('fetched_count', 0)} 真实任免样本 (跨 {len(PROVINCES)} 试点省)",
        "",
        "## 1. 实体逐项 (真实任免样本)",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="642-A.2 M4.5 任免真实抓取")
    args = parser.parse_args()
    results = run_fetch()
    write_outputs(results)
    print(
        f"642-A.2 M4.5: http_count={results['summary']['http_count']}, "
