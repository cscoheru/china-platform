#!/usr/bin/env python3
"""665d generate_seed_hongheiku_timeseries_2024.py — build seed CSV from parse JSON.

Mirror 665c seed CSV schema (dbt seed format) but 5 增量 only:
...
Sources:
  - evidence_pack/u6_batch_y2024_parse_20260904.json (extract 5 增量 per province)
  - evidence_pack/u6_batch_y2024_fetch_20260904.json (URL per province)

Output:
  dbt/seeds/seed_hongheiku_timeseries_2024.csv (~122 rows; 28/28 provinces PARSED;
  3 missing provinces GUIZHOU/HAINAN/LIAONING not included — 沿用 660 红线 DATA_MISSING)

Scope: 5 增量 indicators only (gdp_percapita/fiscal_rev/fixed_asset/retail/trade).
5 现 indicators 不写入此 seed (663 baseline 已 hardcoded in real_2024_provinces).
"""
import csv
import json
...
PROVINCE_NAME_CN = {
    ...
    'JILIN': '吉林', 'LIAONING': '辽宁', 'NEI_MENGGU': '内蒙古', 'QINGHAI': '青海',
    'SHAANXI': '陕西', 'SHANDONG': '山东', 'SHANGHAI': '上海', 'SHANXI': '山西',
    'SICHUAN': '四川', 'TIANJIN': '天津', 'XINJIANG': '新疆', 'XIZANG': '西藏',
...
def main() -> int:
    ...
    for cell in parse_data['cells']:
        ...
        for ind_key, value in cell['extracted'].items():
            label_cn, unit = INDICATOR_LABELS_5[ind_key]
            rows.append({
