#!/usr/bin/env python3
"""669b-i-wuxi mart apply — UPDATE-ONLY psycopg2 pattern.

knife 669b-i WUXI sub-knife 3/4 (2026-09-10): 续刀 batch 1 single-city 6-year harvest.

Per 663 Gap 1 + 669fix-b-2024 pattern — does NOT DROP. Reads seed CSV with 60 rows
(26 real HONGHEIKU_TRANSLOAD + 34 DATA_MISSING), UPDATEs applicable cells with correct
...
WUXI-specific notes:
- 2020: hongheiku 2020 WUXI 公告走 /1707.html 老 ID, 非 /djs/ 标准 pattern (Knife E 不支持)
- 2022: hongheiku tag 页无 2022 /djs/{eid}.html (仅 /xjtjgb/xj2020/34940.html 非标准 path)
...
SEED_CSV = "/tmp/669b_i_cache/seed_wuxi_full.csv"
CITY_CODE = "JIANGSU_WUXI"
WUXI_EID = {2021: 23931, 2023: 45593, 2024: 60801, 2025: 70051}
...
def main():
    print(f"=== knife 669b-i-wuxi mart apply (UPDATE-ONLY) ===")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for indicator, year, value in real_rows:
                ...
                eid = WUXI_EID.get(year, "")
                lineage_origin = f"tjgb.hongheiku.com/djs/{eid}.html" if eid else ""
                lineage_ruling = f"K669b-i-wuxi-parse-{year}-2026-09-10"
                cur.execute(f"""
            ...
            for indicator, year, missing_reason in miss_rows:
                if year == 2024:
                    ...
                    # 2020: hongheiku /1707.html 老 ID, 非 djs pattern
                    lineage_origin = "tjgb.hongheiku.com/1707.html (老 ID 非 djs pattern, Knife E 不支持, 守新增红线-3 不手填)"
                    new_ruling = "K669b-i-wuxi-no-bulletin-djs-2020-2026-09-10"
                elif year == 2022:
                    # 2022: hongheiku tag 页无 /djs/{eid}.html
                    lineage_origin = "tjgb.hongheiku.com/xjtjgb/xj2020/34940.html (非标准 path, Knife E 不支持, 守新增红线-3 不手填)"
                    new_ruling = "K669b-i-wuxi-no-bulletin-2022-2026-09-10"
                elif year == 2026:
                    # 2026: 全 DATA_MISSING 守新增红线-2
                    lineage_origin = "none"
                    new_ruling = "pending"
                else:
                    # 2021/2023/2025: gdp_percapita 或 2021 fiscal_rev parser 未匹配
                    eid = WUXI_EID.get(year, "")
                    lineage_origin = f"tjgb.hongheiku.com/djs/{eid}.html (bulletin 无 gdp_percapita 数据 / 2021 fiscal_rev parser 未匹配)" if eid else ""
                    new_ruling = "K669b-i-wuxi-parse-fixed_asset_growth_pct-2026-09-10"
                cur.execute(f"""
                    UPDATE {TARGET_SCHEMA}.{MART_NAME}
                    SET value = NULL,
                        status = 'DATA_MISSING',
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"SELECT COUNT(*) FROM {TARGET_SCHEMA}.{MART_NAME} WHERE city_code = %s AND lineage_ruling LIKE 'K669b-i-wuxi-%%' AND value IS NOT NULL", (CITY_CODE,))
            ruling_real = cur.fetchone()[0]
            cur.execute(f"SELECT COUNT(*) FROM {TARGET_SCHEMA}.{MART_NAME} WHERE city_code = %s AND lineage_ruling LIKE 'K669b-i-wuxi-%%' AND value IS NULL", (CITY_CODE,))
        ...
        print(f"\n=== Apply summary ===")
        print(f"  WUXI total:         {real_total + miss_total} cells ({real_total} real + {miss_total} missing)")
        print(f"  K669b-i-wuxi:       {ruling_real} real + {ruling_miss} missing")
        print(f"\n  Per-year breakdown:")
